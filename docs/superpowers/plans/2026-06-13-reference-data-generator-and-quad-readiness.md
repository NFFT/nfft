...
**Tech Stack:** Python 3 + `mpmath` (dev dependency, `pip install mpmath`; not a build or runtime dependency), `pytest` for generator self-tests; C99 + CUnit + Autotools for the consumed tests; Markdown for the methodology doc and ADRs.
...
- `tests/refgen/README.md` — how to install deps and regenerate.
...
- Create: `tests/refgen/requirements.txt`
- Create: `tests/refgen/README.md`

- [ ] **Step 0: Pin the dependency (byte-reproducibility contract)**

`tests/refgen/requirements.txt`:
```
mpmath==1.3.0
pytest>=7
```
> The CI drift guard (Phase 4) regenerates and `git diff --exit-code`s, so the
> *formatting* must be byte-stable. `mpmath` is pinned to that end; anyone
> regenerating locally must use this same version or the guard will flag spurious
> diffs. (`random`/Mersenne-Twister and the `nstr` algorithm are otherwise stable
> across all CPython 3.x.) If `mpmath==1.3.0` is unavailable at execution time, pin
> whatever version `pip install mpmath` resolves to and update this file + the README.
...
Run: `cd /workspaces/nfft && python -c "import tests.refgen; print('ok')"`
Expected: `ok`

- [ ] **Step 4: Commit**

```bash
git add tests/refgen/__init__.py tests/refgen/tests/__init__.py \
        tests/refgen/requirements.txt tests/refgen/README.md
...
Run: `cd /workspaces/nfft && python -m pytest tests/refgen/tests/test_transforms.py -q`
...
Run: `cd /workspaces/nfft && python -m pytest tests/refgen/tests/test_io_format.py -q`
...
Run: `cd /workspaces/nfft && python -m pytest tests/refgen/tests/test_io_format.py -q -k registration`
...
python -m tests.refgen.generate --module nfct --precision 40 \
...
Run: `cd /workspaces/nfft && python -m pytest tests/refgen/tests -q`
...
Run:
```bash
cd /workspaces/nfft && python -m tests.refgen.generate --module all --precision 64
...
Run: `cd /workspaces/nfft && python -m pytest tests/refgen/tests/test_io_format.py -q -k quad`
...
Run:
```bash
cd /workspaces/nfft && pip install -r tests/refgen/requirements.txt >/dev/null && \
python -m tests.refgen.generate --module all --precision 64 --seed 1 && \
...
pin) before adding the guard, or the CI job will fail on its first run.
...
Run:
```bash
cd /workspaces/nfft && python -c "import yaml,sys; yaml.safe_load(open('.github/workflows/refgen.yml')); print('yaml ok')"
```
Expected: `yaml ok`. (If PyYAML is unavailable, skip — the diff-guard logic was exercised in Step 1.)
