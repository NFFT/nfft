...
> 3. **gh-pages branch confirmed** as the archive (permanent, accumulating per-PR; `actions/deploy-pages` replaces the site each deploy and artifacts expire).
> 4. **Final-review fixes (CI layer):** the `accuracy-report` job uses only `GITHUB_TOKEN` (no environment); it is **skipped for fork PRs** (read-only token can't post/push); both report steps are `continue-on-error` so the *job* (not just the Check conclusion) never reds; the develop publish is gated to `refs/heads/develop` (main/master must not clobber the baseline); the Check is posted on the PR **head** SHA; and the PR script hardens globs/posts (`nullglob`, `--remove-on-error`, jq `first`, `|| echo` on posts).
> 5. **Fork PRs** get the **emoji comment + Check** via a trusted `workflow_run` companion (`accuracy-report-fork.yml` + `accuracy-fork-report.sh`): default-branch code that consumes the fork build's BMF artifacts (data only), no `gh-pages`/PNG archive.
