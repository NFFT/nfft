...
$$\fun{K_{\V{\eta}}}{\V{\xi}} = \sum_{k = 0}^{\infty} a_k \fun{P_k}{\V{\V{\xi}} \cdot \V{\eta}} =  
...
\end{equation}
 follows easily. When $h$ is restricted to $\interv{(}{0}{1}{)}$ and for fixed $\V{\eta} \in \twosphere$ the function
$Q_h:\twosphere \rightarrow \R$, with
\begin{equation}
  \label{PoissonKernel}
  \nonumber
  \fun{Q_h}{\V{\xi} \cdot \V{\eta}} := \frac{1}{4\pi} \frac{1-h^2}{\paren{1-2\V{\xi} \cdot \V{\eta}+h^2}^{3/2}},
\end{equation}
...
$$
  \int_{\twosphere} \fun{Q_{h}}{\V{\xi} \cdot \V{\eta}} \d w(\V{\xi}) = 1.
$$
Further properties with respect to localization and smoothness are derived in \cite{frgesc}*{pp. 112}.
\end{example}

\begin{example}
  The \emph{Gauss-Weierstraß kernel} $W_{\rho}: \twosphere \rightarrow \R$ is defined by
  $$
  ...
  $$
    \int_{\twosphere} \fun{W_{\rho}}{\V{\xi} \cdot \V{\eta}} \d w(\V{\xi}) = 1.
  $$
...
now a second polynomial $Q \in \Pol_{N}$, $N \in \NZ$ known in advance, we give a fast algorithm first described in \cite{postta97} for computing the 
...
\begin{eqnarray*}
  \paren{\fun{P}{\cos \frac{(2j+1)\pi}{4N}}}_{j=0}^{2N-1} = \V{C}_{2N}^{\transp} \paren{a_{k}}_{k=0}^{2N-1},\\
  \paren{\fun{Q}{\cos \frac{(2j+1)\pi}{4N}}}_{j=0}^{2N-1} = \V{C}_{2N}^{\transp} \paren{b_{k}}_{k=0}^{2N-1},
\end{eqnarray*}
...
compensate for the factors $\varepsilon_{k}^N$ introduced by the matrix $\V{D}_{2N}$.
...
\begin{algorithm}[ht]
  ...
  \begin{algorithmic}
    \STATE  Input:  $N = 2^t\; (t \in \N)$, $\paren{a_{k}}_{k = 0}^{N-1}$, $\paren{\fun{Q}{\cos \frac{(2j+1)\pi}{4N}}}_{0}^{"N-1}$
    \STATE
...
We refer the interested reader to \citelist{\cite{postta} \cite{duro93}} for a detailed description of approximative algorithms for the NFFT and further related references.