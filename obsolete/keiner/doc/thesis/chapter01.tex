...
in boldface. Let now $n \in \N$. As usual, $\V{0}$ is the null vector
$\paren{0,0,\ldots,0}^{\transp} \in \R^{n}$ and $\V{I}_{n}$ is the $n$ 
...
\]
Moreover, $\norm{\cdot}_{1}$, $\norm{\cdot}_{2}$ and $\norm{\cdot}_{\infty}$
denote the usual 1-norm, euclidean or 2-norm, and the infinity- or maximum norm,
...
\end{align*}
For the identity $\fun{\mathcal{R}}{A} = \fun{\mathcal{N}}{A^{\h}}$, we refer
the reader to \cite{bjoerk}.
  ...
  \begin{definition}
    ...
    \begin{enumerate}
	    ...
	    \left\{\begin{array}{l@{\quad \text{if} \quad}l} 
	      ...
	      \end{array}\right.
	    \]
	    \item Furthermore, we define the \emph{iterated locally supported kernels} 
  ...
  \end{definition}  
	...
	\begin{lemma}
	  \item The symbol $\fun{L_{h,\lambda}^{\wedge}}{k}$ can be computed recursively by
	    ...
	    \fun{L_{h,\lambda}^{\wedge}}{k-1} \quad \paren{k \ge 1}
	    \]
