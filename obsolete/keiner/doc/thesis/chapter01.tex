...
\end{equation}
One verifies $\fun{P_{k}}{\pm1} = \paren{\pm1}^{k}$ and $\max_{x \in \interv{[}{-1}{1}{]}} \abs{\fun{P_{k}}{x}} = 1$ (\cite{niuv}*{pp. 45\mdash47}).
...
\begin{equation}
  ...
  \end{split}
\end{equation}
Now induction (see \cite[bel91]) yields
\begin{lemma}
...
\begin{figure}[htbp]
  ...
  \caption{The functions $\abs{Y_{k}^{n}}$ plotted for $k = 0,1,2$, $n = -k,\dots,k$ on the sphere.}
