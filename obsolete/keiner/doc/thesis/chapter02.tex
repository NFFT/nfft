...
Referring to Section \ref{Basics:SphericalHarmonics}, the function $f$ can be developed into an orthogonal expansion in terms of the $\Ln{2}{\twosphere}$-orthogonal basis functions $\set{Y_{k}^n}_{(k,n) \in \mathcal{I}^M}$, i.e.
\begin{equation}
...
A first "naive" approach to evaluating $f$ on $\mathcal{X}$ according to \eqref{NFSFT:FourierExpansion} would be to compute first all $(M+1)^2D$
...
We want to recover the Fourier coefficients $\paren{a_{k}^n}_{(k,n) \in \mathcal{I}^M}$ from 
samples $\V{f}_{\mathcal{X}}$ on set of certain nodes $\mathcal{X} \subset \twosphere$. They are given by the inner products
\[
...
\[
  \int_{0}^{\pi} \fun{P_{k}^{\abs{n}}}{\cos \vtheta} \fun{f_{n}}{\vtheta} \sin \vtheta \; \dx \vtheta = \sum_{l=0}^{2M} \varepsilon_{l}^{2M} w_{l}^{\cc} \fun{P_{k}^{\abs{n}}}{\cos \vtheta_{l}^{\cc}} \fun{f_{n}}{\vtheta_{l}^{\cc}}
\]
...
and weights $w_{l}^{\cc}$. The weights are given by
\[ w_{l}^{\cc} := \frac{1}{2M} \sum_{j=0}^{M} \varepsilon_{j}^{M} \frac{-2}{4j^2-1} \cos\frac{lj\pi}{M}.\]
...
\begin{equation}
  ...
  \end{split}
\end{equation}
follow. Associated with this quadrature formulae, we define the sampling sets
\begin{eqnarray*}
...
% This allows for the derivation of an algorithm of exactly the same asymptotic complexity for computing the adjoint product, too.
...
\[
  ...
  \end{array}\right]
\]
where each matrix $\V{C}^{n}_{d} \in \R^{1 \times (M-n+1)}$ for $n = -M,\ldots,M$ realizes the Clenshaw algorithm (Algorithm \ref{NFSFT:Algorithm:Clenshaw}), acting on the subvector $\V{a}^n$ of $\V{a}$.
They matrices $\V{C}_{d}^n$ can be further decomposed, according to the number of steps in the Clenshaw algorithm by
\begin{eqnarray*}
  \V{C}^{n}_{d}       
  & := & \frac{\sqrt{(2|n|)!}}{2^{|n|} |n|!} 
         \paren{1-\cos^2\vtheta_{d}}^{|n|/2}
    \V{C}_{d,1}^{n} \: \V{C}_{d,2}^{n} \: \ldots \: \V{C}_{d,M-\abs{n}}^{n},\\
  \V{C}_{d,l}^{n}     & := & \encl{[}{\V{I}_{l}, \V{\tilde{e}}_{d,l}}{]} \in \R^{l \times (l+1)},\\
  \V{\tilde{e}}_{d,1} & := & \paren{\alpha_{|n|}^n \vtheta_{d} + \beta_{|n|}^n}^{\transp} \in \R^{1},\\
  \V{\tilde{e}}_{d,l} & := & \paren{0,0,\ldots,\gamma_{l+|n|-1}^n,\alpha_{l+|n|-1}^n \vtheta_{d} +
    \beta_{l+|n|-1}^n}^{\transp} \in \R^{l} \quad \paren{l=2,\ldots,M-\abs{n}}.
\end{eqnarray*}
...
We obtain the sought algorithm for the adjoint transform directly from the factorization, given in the last section. We have to evaluate $\V{\tilde{a}} = \V{Y}^{\h} \: \V{\tilde{f}}$, i.e.
\[
  \V{\tilde{a}}
  =
  \encl{[}{
...
We follow the lines in \cite{postta97} and \cite{kupo02}. To be more precise, the algorithm described performs for a fixed $n$ with $|n| \le M$ a change of basis independent of the nodes, to transform a sum of the form
\begin{equation}
...
\ref {NFSFT:transposedFLFT} be $M \ge 2$. Furthermore, we assume without loss of generality $n \ge 0$. We consider polynomials of the form
\begin{equation}
  \label{NFSFT:GnEven}
...
by applying Algorithm \ref{NFSFT:Algorithm:U} with $K=2^{\tau}$. It can be shown that
...
The first step converts each Fourier coefficent $a_{k}^n$ into a polynomial of degree at most one in Chebyshev representation, i.e. $\V{a}_{k}^{(0)} \in \C^2$. The result 
\[
  \V{a}^{(0)} := 
    \left[\begin{array}{c}
      \V{a}_{0}^{(0)}\\
...
These steps represent the cascade summation applied to associated Legendre functions as in Figure \ref{NFSFT:Figure:CascadeSummation}. In each step, 
half the functions $P_{k}^n$ are eliminated 
...
and the result is added to $a^{(\tau-1)}_{2^{\tau+1}l}$ and $a^{(\tau-1)}_{2^{\tau+1}l+1}$ yielding $a^{(\tau)}_{2^{\tau+1}l}$ and $a^{(\tau)}_{2^{\tau+1}l+1}$. Following this, the output contains 
...
We need to keep the first two polynomials, but with their vectors zero-padded to twice the length. Furthermore, we have to add 
...
\end{align*}   
This follows directly from Algorithm \ref{NFSFT:Algorithm:FLFT_unstab}. The matrix $\V{D}^{(\tau)}_{\text{III}}$ realizes 
...
and finally, a duplication of the result vector in order to permit multiplication with two different 
...
forms the sums for the two rows of the result, and finally, the $\V{D}^{(\tau)}_{\text{II}}$ transforms the newly obtained 
...
We shall mention that the first fast 
        ...
        \ELSE
          \STATE Update $a_{0}^{(t-1),\text{stab}}$ and $a_{1}^{(t-1),\text{stab}}$ from
            \[ 
              a_{2^{\tau+1}l+2^{\tau}}^{(\tau-1)},\ a_{2^{\tau+1}l+2^{\tau}+1}^{(\tau-1)}
            \] 
            using \eqref{NFSFT:StabilizationStep} and Algorithm NFSFT:Algorithm:Utransposed
          \STATE Update $a_{2^{\tau+1}l^{(\tau)}} := a_{2^{\tau+1}l^{(\tau-1)}}$, $a_{2^{\tau+1}l+1}^{(\tau)} := a_{2^{\tau+1}l+1}^{(\tau-1)}$
        \ENDIF
      \ENDFOR
    \ENDFOR
...
into the Fourier coefficients $c_{k}^n$, and $\V{T}$ the FLFT. This results in Algorithm \ref{NFSFT:Algorithm:NFSFT} denoted as 
    ...
    \begin{pgftranslate}{\pgfxy(-3.75,0)}    
	    \begin{pgfmagnify}{0.65}{0.65}
	      ...
	      \pgfputat{\pgfxy(16, 4.5)}{\pgfbox[center,center]{\parbox{7cm}
	      {
	        ...
	        \begin{center}
	          NFFT\\ 
	          $\paren{\bigo{M^2 \log M + \log \frac{1}{\varepsilon} D}}$
	        \end{center}  
	      }}}
	  ...
	  \end{pgftranslate}  
  \end{pgfpicture}
  \caption{The NFSFT algorithm in a schematic overview. The total arithmetic complexity is $\bigo{M^3 \log M + \log \frac{1}{\varepsilon} D}$.}  
\end{figure}
\clearpage
...
%\begin{table}
%  \label{NFSFT:Table:Algorithms}
%  \centering
%  \begin{tabular}{r|r}
%    Algorithm & Complexity (flops) \\
%    naive NDSFT & $\bigo{M^3D}$ \\
%    direct NDSFT & $\bigo{M^2D}$ \\
%    direct NDSFT on arbitrary grids & $\bigo{LM^2 + MLJ}$ \\
%    direct NDSFT on Fourier type grid & $\bigo{LM^2+LM \log M}$ \\
%    direct adjoint NDSFT & $\bigo{M^2D}$ \\
%    NFSFT & $\bigo{M^2 \log^2 M + \log\frac{1}{\varepsilon}D}$ \\
%    adjoint NFSFT & $\bigo{M^2 \log^2 M + \log\frac{1}{\varepsilon}D}$ \\
%  \end{tabular}
