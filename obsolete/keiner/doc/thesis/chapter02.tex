...
\begin{algorithm}[htb]
  ...
  \begin{algorithmic}
    ...
    \STATE \invisible{Input:} $\paren{q_{k}}_{k=0}^{N-1}$, $U_{l} \in \Pi_{N}$ for $l=0,1,2,3$ 
...
\begin{figure}
  \centering
  \begin{pgfpicture}{0cm}{0cm}{5cm}{15cm}
    \pgfsetlinewidth{1.0pt}  
    \begin{pgftranslate}{\pgfxy(-3,0)}    
	    \begin{pgfmagnify}{0.65}{0.65}
	      \pgfputat{\pgfxy(0,19)}{\aknGraph}
	      \pgfputat{\pgfxy(0,12)}{\bknGraph}
	      \pgfputat{\pgfxy(0,5)}{\cknGraph}
	      \pgfputat{\pgfxy(12.6,1)}{\pgfbox[left,center]{\Large$\paren{f_{d}}_{d=0}^{D-1}$}}
	      \pgfputat{\pgfxy(12.6, 8)}{\pgfbox[left,center]{\Large$\paren{c_{k}^n}_{n=-M,\ldots,M;k=\abs{n},\ldots,M}$}}
	      \pgfputat{\pgfxy(12.6,15)}{\pgfbox[left,center]{\Large$\paren{b_{k}^n}_{n=-M,\ldots,M;k=0,\ldots,M}$}}
	      \pgfputat{\pgfxy(12.6,22)}{\pgfbox[center,center]{\Large$\paren{a_{k}^n}_{n=-M,\ldots,M;k=-M,\ldots,M}$}}
	      \pgfputat{\pgfxy(13, 4.5)}{\pgfbox[left,center]{\parbox{4cm}
	      {
	        \Large
	        \begin{center}
	          NFFT \\ 
	      ...
	      \pgfputat{\pgfxy(16, 18.5)}{\pgfbox[center,center]{\parbox{4cm}
	      {
	        ...
	        \begin{center}
	          FLFT \\
	          $\paren{\bigo{M^2 \log^2 M}}$
	        \end{center}
	      }}}
	      \pgfsetstartarrow{\pgfarrowtriangle{6pt}}
	      \pgfsetendarrow{\pgfarrowtriangle{6pt}}
        \pgfline{\pgfxy(8.5, 8)}{\pgfxy(10.5, 8)} 
	      \pgfline{\pgfxy(8.5,15)}{\pgfxy(10.5,15)} 
	      \pgfline{\pgfxy(8.5,22)}{\pgfxy(10.5,22)} 
	      \pgfclearstartarrow
	      \pgfsetlinewidth{6.0pt}  
	      \color{gray}
	      \pgfsetendarrow{\pgfarrowtriangle{9pt}}
	      \pgfline{\pgfxy(13,20)}{\pgfxy(13,17)} 
	      \pgfline{\pgfxy(13,13)}{\pgfxy(13,10)} 
	      \pgfline{\pgfxy(13, 6)}{\pgfxy(13,3)} 
	    \end{pgfmagnify}  
	  \end{pgftranslate}  
  \end{pgfpicture}

  \caption{The NFSFT in a schematic overview. The total arithmetic complexity is $\bigo{M^2 \log^2 M + \log \frac{1}{\varepsilon} D}$.}  
\end{figure}
