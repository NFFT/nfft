\chapter{Discrete Spherical Fourier Transform}
\label{DSFT}
Let in this chapter $M \in \NZ$, $M \ge 4$ be a fixed bandwidth, $N := 2^{\ceil{\log_2 M}}$ 
and $\paren{\vtheta_j,\vphi_j}_{j=0,\ldots,D-1}$ a set of arbitrary nodes on $\twosphere$.
...
\end{equation}  
at arbitrary nodes $\paren{\vtheta_j,\vphi_j}_{j=0,\ldots,D-1}$, where 
$$I^M := \pset{\paren{k,n}}{|}{k=0,\ldots,M;n=-k,\ldots,k}.$$
The coefficients $a_k^n$ are the \emph{spherical Fourier coefficients} 
with respect to the orthonormal basis of spherical harmonics $Y_{k}^n$. The index $k$ denotes the degree and $n$ the order. 
By defining the total order $$ \paren{k_1,n_1} \le \paren{k_2,n_2} \iff k_1 \le k_2 \text{ or } n_1 \le n_2 $$ 
over the index set $I^M$ we can
...
\begin{equation}
  \nonumber
  \begin{split}
    \V{f} & := \paren{f_j}_{j=0}^{D-1},\quad \V{Y} := \paren{\fun{Y_l}{\vtheta_j,\vphi_j}}_{j=0,\ldots,D-1, l=0,\ldots,(M+1)^2-1},\\
    \V{a} & := \paren{a_l}_{l=0,\ldots,(M+1)^2-1}.
  \end{split}
\end{equation}

Concerning a \emph{discrete Fourier transform} on the torus $\mathbb{T}$ of length $N \in \N$ which an be represented as a maxtrix-vector product
$$
  \V{f} = \V{F}_{N} \: \V{a}, \quad \V{F}_{N} \in \C^{N \times N},\ \V{a}, \V{f} \in \C^{N}
$$
as well, the Fourier matrix $\V{F}$ is unitary. Henceforth we immediately obtain the inversion formula
$$
  \V{a} = \V{F}^{\h} \: \V{f}
$$ 
The DFT evaluates the trigonometric polynomial 
$$
  f := \sum_{k=0}^{N-1} a_{k} \e^{\im k t}
$$
at equidistant nodes $\paren{t_{j}}_{j=0}^{N-1}$ with $t_{j} := \e{j * \frac{2 \pi}{N}}$. With respect to nonequispaced nodes, the 
matrixcorresponding Fourier matrix is no longer unitary.

Similarly, there exists a grid of nodes on the sphere that renders the matrix $\V{Y}$ unitary. We want to recover the 
Fourier coefficients $\paren{a_{k}^n}_{(k,n) \in I^M}$ of a bandlimited function $f$ on $\twosphere$ from samples 
$\fun{f}{\vtheta,\vphi}$ at certain nodes. The Fourier coefficients are given by
$$
...
$$
By seperating the integration variables, we obtain
\begin{eqnarray*}
