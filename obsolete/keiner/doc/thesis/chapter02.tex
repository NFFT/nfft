...
for odd $n$ with given complex coefficients $a_k^n$. The Fast Legendre Function Transform (FLFT) performes a change of basis and computes the 
...
$$
  ...
  \end{array}\right).   
$$
For the rest of this section restrict ourselves to the case when $n$ is even. Furthermore, we set $a_k^n := 0$ for $k < \abs{n}$ or $k > M$. In a first step, we use Lemma 
