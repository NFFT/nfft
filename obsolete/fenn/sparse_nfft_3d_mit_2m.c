...
void sparse_nfft_trafo_3d(sparse_nfft_plan_3d *this)
{
  ...
  if (this->center_nfft_plan->N[0]<=2*this->center_nfft_plan->m) 
    ndft_trafo(this->center_nfft_plan);
  else
    nfft_trafo(this->center_nfft_plan);
  ...
  for(rr=0;rr<=(J+1)/2;rr++)
	...
	this->act_nfft_plan->N[1]=int_2_pow(r);
      ...
      //printf("\n\n%d x %d x %d:\t",this->act_nfft_plan->N[0],this->act_nfft_plan->N[1],this->act_nfft_plan->N[2]);
	...
	SWAP(this->act_nfft_plan->x,this->x_120);
      ...
      if(this->act_nfft_plan->N[0]<=2*this->act_nfft_plan->m)
	if(this->act_nfft_plan->N[1]<=2*this->act_nfft_plan->m)
	  if(this->act_nfft_plan->N[2]<=2*this->act_nfft_plan->m)
	    ndft_trafo(this->act_nfft_plan);
	  else
...
void sparse_nfft_init(sparse_nfft_plan *this, int J, int M, int m, unsigned snfft_flags)
{
  ...
  this->set_fftw_plan2=(fftw_plan*) fftw_malloc((J/2+1)*sizeof(fftw_plan));
  ...
  nfft_init_specific(this->act_nfft_plan,2,N,M,n,m, MALLOC_X| MALLOC_F| FFTW_INIT, FFTW_MEASURE);

  this->set_fftw_plan1[0]=this->act_nfft_plan->my_fftw_plan1;
  this->set_fftw_plan2[0]=this->act_nfft_plan->my_fftw_plan2;
  ...
  for(r=1;r<=J/2;r++)
	...
	fftw_plan_dft(2, n, this->act_nfft_plan->g1, this->act_nfft_plan->g2,
      ...
      this->set_fftw_plan2[r] = 
	fftw_plan_dft(2, n, this->act_nfft_plan->g2, this->act_nfft_plan->g1,
		      FFTW_BACKWARD, this->act_nfft_plan->fftw_flags);
    }
...
void sparse_nfft_init_3d(sparse_nfft_plan_3d *this, int J, int M, int m, unsigned snfft_flags)
{
  ...
  this->set_nfft_plan_1d = (nfft_plan*) fftw_malloc((ld(2*m)+1)*(sizeof(nfft_plan)));
  this->set_nfft_plan_2d = (nfft_plan*) fftw_malloc((ld(2*m)+1)*(sizeof(nfft_plan)));
  ...
  nfft_init_specific(this->act_nfft_plan,3,N,M,n,m, MALLOC_X| MALLOC_F, FFTW_MEASURE);

  /* malloc g1, g2 for maximal size */
  this->act_nfft_plan->g1 = fftw_malloc(this->sigma*this->sigma*this->sigma*int_2_pow(J/2)*int_2_pow(J/2)*int_2_pow(J-J/2)*sizeof(fftw_complex));
  this->act_nfft_plan->g2 = fftw_malloc(this->sigma*this->sigma*this->sigma*int_2_pow(J/2)*int_2_pow(J/2)*int_2_pow(J-J/2)*sizeof(fftw_complex));

  this->act_nfft_plan->my_fftw_plan1 =
    fftw_plan_dft(3, n, this->act_nfft_plan->g1, this->act_nfft_plan->g2,
		  FFTW_FORWARD, this->act_nfft_plan->fftw_flags);
  
  this->act_nfft_plan->my_fftw_plan2 = 
    fftw_plan_dft(3, n, this->act_nfft_plan->g2, this->act_nfft_plan->g1,
		  FFTW_BACKWARD, this->act_nfft_plan->fftw_flags);
  ...
  for(r=1;r<=J/2;r++)
    {
      N[0]=int_2_pow(r);   n[0]=this->sigma*N[0];
      N[1]=int_2_pow(r);   n[1]=this->sigma*N[1];
      N[2]=int_2_pow(J-r); n[2]=this->sigma*N[2];
      this->set_fftw_plan1[r] =
	fftw_plan_dft(3, n, this->act_nfft_plan->g1, this->act_nfft_plan->g2,
		      FFTW_FORWARD, this->act_nfft_plan->fftw_flags);
...
void sparse_nfft_finalize(sparse_nfft_plan *this)
{
  ...
  this->act_nfft_plan->my_fftw_plan2=this->set_fftw_plan2[0];

  for(r=1;r<=this->J/2;r++)
    {
      fftw_destroy_plan(this->set_fftw_plan1[r]);
      fftw_destroy_plan(this->set_fftw_plan2[r]);
    }
...
void sparse_nfft_finalize_3d(sparse_nfft_plan_3d *this)
{
  ...
  /* the 1d and 2d nffts */
  for(r=0;r<=ld(2*this->act_nfft_plan->m);r++)
    {
      if(int_2_pow(this->J-r)>2*this->act_nfft_plan->m)
	{
