...
Nothing in the library offers the dyadic ladder. `X(tune_plan)` walks every
even 5-smooth `n` in `[5/4 N, 4N]` and must carry a radix-2 tie-break to undo
the FFT-size penalty that walk creates.
...
Return codes match `X(tune_plan)`: 1 goal met, 0 capped goal met instead,
-1 bad arguments.
...
- `X(tune_plan)` and its constants must not change.
