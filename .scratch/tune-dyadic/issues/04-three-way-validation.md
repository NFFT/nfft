# 04 — Three-way validation on an extended N set

Status: done

## Problem

`compare.c` runs two arms, `X(tune_plan)` against the legacy grid with an
oracle cut-off, over N in {243, 250, 251, 255, 256, 512}. Every one of those
has `t = next_power_of_2(N)/N <= 1.06`, so the dyadic ladder's rung 0 is
illegal in all 288 cases and the new tuner would return the legacy grid every
time. The existing data cannot decide anything about this work.

## What to do

Add a third arm to `compare.c` for `X(tune_plan_dyadic)` and extend the N set
to span `t` across (1, 2]:
...
Extend `compare_report.py` and `docs/tuning-vs-legacy.md` with the third arm,
split by shape and by `t` band (`t < 5/4` against `t >= 5/4`) — the split is
the whole point, since the two tuners are identical below the threshold.
