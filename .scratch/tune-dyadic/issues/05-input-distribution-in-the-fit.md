...
library's own accuracy tests and by `Y(tune_refine)`'s probes — draws them on
...
`X(tune_plan)`'s shipped constants carry the same exposure. They pass today
because a single envelope over sigma in [5/4, 4] happens to leave 2x or more
of slack at the geometries the tests exercise. That is luck, not design.
...
`X(tune_plan)`'s constants were fitted on the centred data and have not been
refitted. Doing so would change shipped behaviour for the existing tuner and
is out of scope for the dyadic work; it should be its own change.
