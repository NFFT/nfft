/* Kaiser-Bessel sigma/m accuracy sweep.
 *
 * For a grid of (sigma, N, m) it measures the NFFT error in the standard
 * measure used by tests/nfft.c,
 *
 *   forward: max_j |f_j - s_j| / sum_k |f_hat_k|
 *   adjoint: max_k |fhat_k - s_k| / sum_j |f_j|
 *
 * against a long-double direct NDFT reference, and writes one CSV row per
 * (sigma, N, m, trial). Build one binary per precision; the precision is
 * selected by -DSWEEP_PREC={0,1,2}.
 */

#include <complex.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#include "nfft3.h"

#if SWEEP_PREC == 0
typedef float R;
typedef float _Complex C;
#define NF(name) nfftf_##name
#define PREC_NAME "float"
#define CRE crealf
#define CIM cimagf
#elif SWEEP_PREC == 1
typedef double R;
typedef double _Complex C;
#define NF(name) nfft_##name
#define PREC_NAME "double"
#define CRE creal
#define CIM cimag
#elif SWEEP_PREC == 2
typedef long double R;
typedef long double _Complex C;
#define NF(name) nfftl_##name
#define PREC_NAME "long_double"
#define CRE creall
#define CIM cimagl
#else
#error set SWEEP_PREC
#endif

typedef long double LR;
typedef long double _Complex LC;

#define LPI 3.141592653589793238462643383279502884L

/* xorshift64*, so the grid is reproducible independently of libc. */
static unsigned long long rng_state;
static void rng_seed(unsigned long long s)
{
  rng_state = s ? s : 88172645463325252ULL;
}
static double rng_uniform(void)
{
  rng_state ^= rng_state >> 12;
  rng_state ^= rng_state << 25;
  rng_state ^= rng_state >> 27;
  return (double)((rng_state * 2685821657736338717ULL) >> 11) /
         9007199254740992.0;
}

/* exp(-2 pi i k x) with the phase reduced before the trig call. */
static LC twiddle(LR k, LR x, int sign)
{
  LR t = fmodl(k * x, 1.0L);
  LR phi = 2.0L * LPI * t;
  return cosl(phi) + (LR)sign * I * sinl(phi);
}

static void ndft_forward(int Ntot, int M, const LR *x, const LC *f_hat, LC *f)
{
  int j, k;
  const LR k0 = (LR)(-Ntot / 2);
  for (j = 0; j < M; j++)
  {
    LC v = 0.0L;
    for (k = 0; k < Ntot; k++)
      v += f_hat[k] * twiddle(k0 + (LR)k, x[j], -1);
    f[j] = v;
  }
}

static void ndft_adjoint(int Ntot, int M, const LR *x, const LC *f, LC *f_hat)
{
  int j, k;
  const LR k0 = (LR)(-Ntot / 2);
  for (k = 0; k < Ntot; k++)
  {
    LC v = 0.0L;
    for (j = 0; j < M; j++)
      v += f[j] * twiddle(k0 + (LR)k, x[j], +1);
    f_hat[k] = v;
  }
}

int main(int argc, char **argv)
{
  /* The tune contract floors sigma at 1.25; the grid starts there. */
  static const double sigmas[] = {1.25, 1.3,  1.35, 1.4, 1.5, 1.6, 1.75,
                                  2.0,  2.25, 2.5,  3.0, 3.5, 4.0};
  static const int Ns[] = {64, 256, 1024};
  const int n_sigma = (int)(sizeof(sigmas) / sizeof(sigmas[0]));
  const int n_N = (int)(sizeof(Ns) / sizeof(Ns[0]));
  const int trials = 3;
  const int m_max_cap = (argc > 1) ? atoi(argv[1]) : 40;

  int is, iN, m, t, j, k;

  printf("prec,sigma_req,sigma,N,n,M,m,trial,err_fwd,err_adj\n");

  for (is = 0; is < n_sigma; is++)
    for (iN = 0; iN < n_N; iN++)
    {
      const int N = Ns[iN];
      int n = (int)(sigmas[is] * N + 0.5);
      const int M = 2 * N;
      NFFT_INT Nl, nl;
      LR *xl;
      LC *fhat_l, *f_l, *ref_f, *ref_fhat;
      R *x;
      C *fhat_in, *f_in, *f_out, *fhat_out;
      LR nrm1_fhat, nrm1_f;
      int m_max;

      if (n % 2)
        n++;
      if (n <= N)
        n = N + 2;
      Nl = (NFFT_INT)N;
      nl = (NFFT_INT)n;
      m_max = n / 2 - 1;
      if (m_max > m_max_cap)
        m_max = m_max_cap;

      xl = (LR *)malloc((size_t)M * sizeof(LR));
      fhat_l = (LC *)malloc((size_t)N * sizeof(LC));
      f_l = (LC *)malloc((size_t)M * sizeof(LC));
      ref_f = (LC *)malloc((size_t)M * sizeof(LC));
      ref_fhat = (LC *)malloc((size_t)N * sizeof(LC));
      x = (R *)NF(malloc)((size_t)M * sizeof(R));
      fhat_in = (C *)NF(malloc)((size_t)N * sizeof(C));
      f_in = (C *)NF(malloc)((size_t)M * sizeof(C));
      f_out = (C *)NF(malloc)((size_t)M * sizeof(C));
      fhat_out = (C *)NF(malloc)((size_t)N * sizeof(C));

      for (t = 0; t < trials; t++)
      {
        rng_seed(0x5eed0000ULL +
                 (unsigned long long)(is * 1000 + iN * 100 + t));

        for (j = 0; j < M; j++)
        {
          xl[j] = (LR)(rng_uniform() - 0.5);
          x[j] = (R)xl[j];
        }
        /* Round-trip through the working precision so the reference sees the
         * same data the transform does. */
        nrm1_fhat = 0.0L;
        for (k = 0; k < N; k++)
        {
          R re = (R)(rng_uniform() - 0.5), im = (R)(rng_uniform() - 0.5);
          fhat_in[k] = re + I * im;
          fhat_l[k] = (LR)re + I * (LR)im;
          nrm1_fhat += cabsl(fhat_l[k]);
        }
        nrm1_f = 0.0L;
        for (j = 0; j < M; j++)
        {
          R re = (R)(rng_uniform() - 0.5), im = (R)(rng_uniform() - 0.5);
          f_in[j] = re + I * im;
          f_l[j] = (LR)re + I * (LR)im;
          nrm1_f += cabsl(f_l[j]);
        }

        ndft_forward(N, M, xl, fhat_l, ref_f);
        ndft_adjoint(N, M, xl, f_l, ref_fhat);

        for (m = 1; m <= m_max; m++)
        {
          LR e_fwd = 0.0L, e_adj = 0.0L;
          NF(plan_ng) *p = NF(plan_ng_guru)(
              1, &Nl, NULL, &nl, (NFFT_INT)M, m, NFFT_WINDOW_KAISER_BESSEL, x,
              (void *)fhat_out, (void *)f_out, 0u,
              NFFT_ESTIMATE | NFFT_NO_DIRECT);
          if (!p)
          {
            fprintf(stderr, "guru failed N=%d n=%d m=%d\n", N, n, m);
            continue;
          }
          NF(precompute)(p);

          NF(execute_on)(p, (void *)fhat_in, (void *)f_out);
          for (j = 0; j < M; j++)
          {
            LC a = (LR)CRE(f_out[j]) + I * (LR)CIM(f_out[j]);
            LR e = cabsl(a - ref_f[j]);
            if (e > e_fwd)
              e_fwd = e;
          }

          NF(execute_adjoint_on)(p, (void *)fhat_out, (void *)f_in);
          for (k = 0; k < N; k++)
          {
            LC a = (LR)CRE(fhat_out[k]) + I * (LR)CIM(fhat_out[k]);
            LR e = cabsl(a - ref_fhat[k]);
            if (e > e_adj)
              e_adj = e;
          }

          NF(plan_ng_destroy)(p);

          printf("%s,%.6f,%.10f,%d,%d,%d,%d,%d,%.10Le,%.10Le\n", PREC_NAME,
                 sigmas[is], (double)n / (double)N, N, n, M, m, t,
                 e_fwd / nrm1_fhat, e_adj / nrm1_f);
        }
        fflush(stdout);
      }

      free(xl);
      free(fhat_l);
      free(f_l);
      free(ref_f);
      free(ref_fhat);
      NF(free)(x);
      NF(free)(fhat_in);
      NF(free)(f_in);
      NF(free)(f_out);
      NF(free)(fhat_out);
    }

  return 0;
}
