...
The CSV comes from compare.c, which now runs three arms over the same nodes,
the same data and the same plan_ng path, so only the parameter pair differs:
    ...
    for p in paths:
        with open(p) as fh:
            for r in csv.DictReader(fh):
                for k in ("N", "M", "n_new", "m_new", "n_dya", "m_dya",
                          "n_leg", "m_leg", "new_met", "dya_met", "leg_met"):
                    r[k] = int(r[k])
                for k in ("goal", "err_new", "err_dya", "err_leg", "t_new",
                          "t_dya", "t_leg"):
                    r[k] = float(r[k])
...
def main():
    ...
    w("- **tune_plan** -- `nfft_tune_plan`, every even 5-smooth `n` in")
    w("  `sigma` of `[5/4, 4]`.")
    w("- **dyadic** -- `nfft_tune_plan_dyadic`, the three rungs")
    w("  `n = 2^j * next_power_of_2(N)`.")
    w("")
    w("Speedups are against legacy, so above 1.00x is a win. Both tuners run")
