...
#define MACRO_ndct_init__cos_vec {					\
    ...
    for( t = 0; t < ths->d; t++) {					\
      ...
      for( k = 2; k < ths->N[t]; k++)					\
	cos_vec[t][k] = 2.0 * cos_vec[t][1] * cos_vec[t][k-1] - cos_vec[t][k-2]; \
    }									\
  }
...
#define MACRO_nfct_D_init_result_A  memset(g_hat,   0, ths->dct_n_total * sizeof( double));
...
#define MACRO_update__phi_inv_k__kg_plain( what_kind, which_phi) {	\
	...
	kg_plain[t+1]    = kg_plain[t]  * ths->dct_n[t] +  kg[t];      \
      }                                                                 \
  } 
...
#define MACRO_nfct_B_init_result_T   memset(g, 0, ths->dct_n_total * sizeof( double));
...
#define MACRO_count__lg(dim) {						\
    ...
    if( (lg[(dim)] == 0) || (lg[(dim)] == ths->dct_n[(dim)]-1))	\
      count_lg[(dim)] *= -1;						\
    /* move array index */						\
    lg[(dim)] += count_lg[(dim)];					\
  }
...
#define  MACRO_update_phi_tilde_lg_plain( which_one, which_psi) {	\
	...
	lg_plain[t+1]  = lg_plain[t]  * ths->dct_n[t] + lg[t];		\
      }									\
  }
...
void nfct_transposed( nfct_plan *ths) {
  ...
  /* undo changes */
//   for( a = 0; a < ths->dct_n_total; a += ths->dct_n)
//     ths->g_hat[a] = 2.0 * ths->g_hat[a];
...
void nfct_init_help(nfct_plan *ths)
{
  ...
  //ths->n_total = nfct_prod_int(ths->n, ths->d);
  ths->dct_n_total = nfct_prod_int(ths->dct_n, ths->d);
  ...
  /* NO FFTW_MALLOC HERE */
  if(ths->nfct_flags & PRE_PSI)
    ths->psi = (double*) malloc(ths->M_total * ths->d * NFCT_SUMMANDS * sizeof(double));
  
  if(ths->nfct_flags & FFTW_INIT)
    {
      ths->g1 = (double*)fftw_malloc(ths->dct_n_total * sizeof(double));

      if(ths->nfct_flags & FFT_OUT_OF_PLACE)
	{
	  ths->g2 = (double*) fftw_malloc(ths->dct_n_total * sizeof(double));
	}
      else
	ths->g2 = ths->g1;

      ths->my_fftw_r2r_plan = fftw_plan_r2r( ths->d, ths->dct_n, ths->g1, ths->g2, 
...
void nfct_init( nfct_plan *ths, int d, int *N, int M)
{
  int t;                                /**< index over all dimensions        */
  ...
  ths->dct_n = (int*) fftw_malloc( d * sizeof( int));
  ths->n = (int*) fftw_malloc( d * sizeof( int));

  for( t = 0; t < d; t++)
    {
      ths->n[t] = 4 * next_power_of_2( ths->N[t]);
      ths->dct_n[t] = ths->n[t] / 2 + 1;
    }
...
void nfct_init_guru( nfct_plan *ths, int d, int *N, int M, int *n,
		     int m, unsigned nfct_flags, unsigned fftw_flags)
{
  ...
  ths->M_total = M;
  ths->n = (int*)fftw_malloc( ths->d * sizeof( int));
  ths->dct_n = (int*)fftw_malloc( ths->d * sizeof( int));
  for( t = 0; t < d; t++)
    {
      ths->n[t] = n[t];
      ths->dct_n[t] = (n[t] / 2) + 1;
...
void nfct_init_1d(nfct_plan *ths, int N1, int M)
{
  int N[1];
...
void nfct_init_2d(nfct_plan *ths, int N1, int N2, int M)
{
  int N[2];
...
void nfct_init_3d(nfct_plan *ths, int N1, int N2, int N3, int M)
{
  int N[3];
