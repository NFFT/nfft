...
#define MACRO_ndct_init_result_trafo   memset( f, 0, ths->M_total * sizeof( double))
#define MACRO_ndct_init_result_transposed   memset( f_hat, 0, ths->N_total * sizeof( double));
...
#define MACRO_ndct_malloc__cos_vec  					\
    ...
    for( t = 0; t < ths->d; t++)                                        \
      cos_vec[t] = (double*) malloc( ths->nfct_N[t] * sizeof( double));	\
...
#define MACRO_ndct_compute__transposed {	\
    						\
...
#define MACRO_ndct(which_one)					  \
  ...
  } /* ndct_{trafo,transposed} */

 
MACRO_ndct(trafo)
MACRO_ndct(transposed)
 ...
 *  f[j] = sum_{k in I_N^d} f_hat[k] * cos(2 (pi) k x[j])
 *
 * direct computation of the nfct_transposed, formula (1.2)
 * nfct_transposed:
 * for k in I_N^d
 *  f_hat[k] = sum_{j=0}^{M-1} f[j] * cos(2 (pi) k x[j])
 */
...
#define MACRO_nfct_D_init_result_A  memset(g_hat,   0, nfct_prod_int(ths->nfct_n, ths->d) * sizeof( double));

#define MACRO_nfct_D_init_result_T  memset(f_hat, 0, ths->N_total     * sizeof( double));



#define MACRO_with_PRE_PHI_HUT * ths->c_phi_inv[t][kg[t]];

#define MACRO_compute_PHI_HUT / (PHI_HUT( kg[t], t))
...
#define MACRO_update__phi_inv_k__kg_plain( what_kind, which_phi) {	\
  ...
  for( t = i; t < ths->d; t++) {                                        \
    ...
    kg_plain[t+1] = kg_plain[t] * ths->nfct_n[t] + kg[t];               \
  }                                                                     \
} 
...
inline void nfct_D_ ## which_one (nfct_plan *ths) {			\
  ...
  if( ths->nfct_flags & PRE_PHI_HUT)					\
    ...
    for( k_L = 0; k_L < ths->N_total; k_L++) {				\
      									\
      MACRO_update__phi_inv_k__kg_plain( which_one, compute_PHI_HUT);	\
      									\
      MACRO_nfct_D_compute_ ## which_one;				\
      									\
      MACRO_count__kg;							\
 ...
 */ 
#define MACRO_nfct_B_init_result_A   memset(f, 0, ths->M_total * sizeof( double));

#define MACRO_nfct_B_init_result_T   memset(g, 0, nfct_prod_int( ths->nfct_n, ths->d) * sizeof( double));
...
#define MACRO_count__lg(dim) {						\
  ...
  if( (lg[(dim)] == 0) || (lg[(dim)] == ths->nfct_n[(dim)]-1))	        \
    count_lg[(dim)] *= -1;						\
  /* move array index */						\
  lg[(dim)] += count_lg[(dim)];					        \
}
...
#define  MACRO_update_phi_tilde_lg_plain( which_one, which_psi) {	\
  for( t = i; t < ths->d; t++) {					\
    ...
    lg_plain[t+1]  = lg_plain[t]  * ths->nfct_n[t] + lg[t];		\
  }									\
}
...
void nfct_transposed( nfct_plan *ths) {
 ...
 *  used in autimatically generated infct_* functions
 **/
...
void nfct_precompute_phi_hut( nfct_plan *ths)
{
  ...
  for( t = 0; t < ths->d; t++) {
    ...
    ths->c_phi_inv[t] = (double*)fftw_malloc( ths->nfct_N[t] * sizeof( double));
   
    for( kg[t] = 0; kg[t] < ths->nfct_N[t]; kg[t]++)  {
      /*                       = 1.0 / PHI_HUT */
      ths->c_phi_inv[t][kg[t]] = 1.0 MACRO_compute_PHI_HUT;
      //DELME      printf("%d/%d/%d  %e\n", t, ths->m, ths->N[t], (PHI_HUT(kg[t], t)));
    }
  }
} /* nfct_phi_hut */
...
void nfct_init_help( nfct_plan *ths)
{
  ...
  ths->N_total = nfct_prod_int( ths->nfct_N, ths->d);
  ...
  if(ths->nfct_flags & FFTW_INIT) {
    ths->g1 =
      (double*)fftw_malloc( nfct_prod_int(ths->nfct_n, ths->d) * sizeof( double));
    ...
    ths->my_fftw_r2r_plan = 
      fftw_plan_r2r( ths->d, ths->nfct_n, ths->g1, ths->g2, 
...
void nfct_init( nfct_plan *ths, int d, int *nfct_N, int M_total) {
  ...
  ths->nfct_N = (int*) fftw_malloc( ths->d * sizeof( int));
  ...
  for( t = 0; t < d; t++) {
    ths->nfct_n[t] = 2 * next_power_of_2( ths->nfct_N[t]) + 1;
    ths->n[t]      = 2 * next_power_of_2( ths->N[t]);
  }

  WINDOW_HELP_ESTIMATE_m;

  ths->nfct_flags = PRE_PSI| PRE_PHI_HUT| MALLOC_X| MALLOC_F_HAT| MALLOC_F| FFTW_INIT| FFT_OUT_OF_PLACE;
...
void nfct_init_guru( nfct_plan *ths, int d, int *nfct_N,
		     int M_total, int *nfct_n, int m,
...
void nfct_init_1d( nfct_plan *ths, int nfct_N0, int M_total) {

  int nfct_N[1];

  nfct_N[0] = nfct_N0;
  nfct_init( ths, 1, nfct_N, M_total);
}

void nfct_init_2d( nfct_plan *ths, int nfct_N0, int nfct_N1, int M_total) {

  int nfct_N[2];

  nfct_N[0] = nfct_N0;
  nfct_N[1] = nfct_N1;
  nfct_init( ths, 2, nfct_N, M_total);
}

void nfct_init_3d( nfct_plan *ths, int nfct_N0, int nfct_N1, int nfct_N2, int M_total) {
  ...
  nfct_N[1] = nfct_N1;
  nfct_N[2] = nfct_N2;
  nfct_init( ths, 3, nfct_N, M_total);
}
