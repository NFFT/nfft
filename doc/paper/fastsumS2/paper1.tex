...
precomputation step of the fast summation algorithm. We do not focus on this
method and the error it introduces, since all considered Fourier-Legendre
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}
    ...
    \STATE
    \STATE Output: $\paren{\fun{f_M}{\V{\xi}_{d}}}_{d=0}^{D-1}$ approximating $\paren{\fun{f}{\V{\xi}_{d}}}_{d=0}^{D-1}$
    \STATE
...
\cite{bahu01,Sc97}, i.e., for a fixed set of parameters the coefficients
$\fun{K^{\wedge}}{k}$ obey a certain decay rate with respect to $k$.
In contrast, we use the decay of the Fourier-Legendre coefficients with respect
to $k$ and the parameters of the kernel to determine the truncation index $M$
...
\begin{corollary}
  ...
  \pset{\V{\eta}_{l} \in \twosphere}{|}{l = 0,\ldots,L-1}$, a set of target
  nodes $\mathcal{X} := \pset{\V{\xi}_{d} \in \twosphere}{|}{d=0,\ldots,L-1}$,
  and the matrix $\V{K}=(\fun{K}{\V{\eta}_{l} \cdot \V{\xi}_d})_{d,l=
    0,\hdots,L-1}$ be given.
  Then, the poposed approximation, see also Remark \ref{rem:rankM}, fulfils
  \begin{equation*}
