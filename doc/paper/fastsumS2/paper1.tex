...
precomputation step of the fast summation algorithm. We do not focus on the
introduced error, since all considered symbols obey an exact precomputation
scheme.
\end{remark}
...
against the decay of its symbol, see \cite{Sc97} for details.
\end{remark}
...
The \emph{panel clustering} method applied to \eqref{Applications:KernelSum}
in \cite{FrGlSch98} reduces the computational effort to evaluate
\eqref{Applications:KernelSum} based on the traditional method of dividing the
evaluation into near- and far-field.
...
Fast Spherical Fourier Transform (adjoint NFSFT) in $\mathcal{O}(L + M^2
\log^2 M)$ arithmetic operations, see \cite{kupo02, keiner05} for details.
This is followed by $(M+1)^2$ multiplications with the symbol
$\fun{K^{\wedge}}{k}$, and completed by a NFSFT to compute the outer sum for
the target nodes $\mathcal{X}$ in $\mathcal{O}(D + M^2 \log^2 M)$ arithmetic
operations.
...
$b_l$ and a decay condition on the symbol of the particular kernel.
In contrast to results given in \cite{bahu01,Sc97} we are interested also in
the decay of the symbol in relation to the parameter of the kernel.
...
symbol is neccesary for the fast summation algorithm.
...
\begin{lemma}
 \begin{enumerate}
   \item 
Using the Poisson kernel $K=Q_h$ within our summation algorithm yields an relative error of
     \begin{equation}
       \label{error:poisson}
     ...
     \end{equation}
     \item 
Using the singularity kernel $K=S_h$ within our summation algorithm yields an relative error of
       \begin{equation}
...
For the Gaussian-kernel $G_{\sigma}$ holds:
  \begin{enumerate}
  \item The symbol $\fun{G_{\sigma}^{\wedge}}{k}$ fulfils a highly unstable
    recurrence relation
    \[
...
\begin{proof}
  \begin{enumerate}
  \item The relation \eqref{three2} and integration by parts yield the
  assertion.
