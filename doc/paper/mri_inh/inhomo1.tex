...
The reconstruction algorithms were tested with a $2d$-Shepp-Logan phantom, cf. Figure \ref{Fig:Phantom}, of size $256\times256$ with a simulated field map. Furthermore we use MR measurements of a physical phantom by the Philips Achieva 1.5T device with a measured field maps. In all test we use an interleaved spiral k-space trajectory, given by
\begin{equation*}
...
In all tests we used the oversampling factor $\alpha \approx 1.5$, such that $\alpha N_1=\alpha N_2 = 384$. The factor $N_3$ depends on the field map. With the simulated field map we choose $N_3=40$, with the measured field map $N_3=80$ A greater oversampling factor results in better SNR but not in a visible better image quality. 
...
NFFT, i.e., the left factor $\zb B$, see Section \ref{Sec:NFFT}. For better comparison we choose in the tests \label{Exp:1} and \label{Exp:3} the same
flag PRE\_PSI which stores the precomputed values in a lossless compressed form.
However, the NFFT2 library can also store all precomputed values of the left factor $\zb B$ in an uncompressed form (flag PRE\_PSI) which results in better performance but higher memory consumption. In experiment \ref{Exp:2} we test different flags. 



\begin{NumExp} \label{Exp:1}
First of all, we compare the different reconstruction methods with a $2d$-Shepp-Logan phantom. The simulated MR data are computed by the $2d\otimes1d$-method using the following field map
\begin{equation*}
...
\begin{tabular}{|l||c|c|c|c|c|} \hline
 ...
 \rule{0mm}{4mm} 2d NFFT & 4.14 & 4.40 & 4.44  & 4.47 & 31 \\[1ex] \hline
 \rule{0mm}{4mm} 3d NNFFT & 10.15 & 13.55 & 14.02 & 14.43 & 32 \\[1ex] \hline
 \rule{0mm}{4mm} 3d NFFT & 10.15 & 13.55 & 14.02 & 14.43 & 256 \\[1ex] \hline
 \rule{0mm}{4mm} 2d$\otimes$1d NFFT & 9.80 & 13.45 & 14.00 & 14.46& 1394 \\[1ex] \hline \hline
 \multicolumn{5}{|c|}{\rule{0mm}{4mm} CPU-Time} & CPU-Time/Iter\\[1ex] \hline\hline
 \rule{0mm}{4mm} 2d NFFT & 3.27 & 4.91 & 9.83 & 18.07 &  \\[1ex] \hline
 \rule{0mm}{4mm} 3d NNFFT & 140.04 & 210.24 & 421.26 & 773.46&  \\[1ex] \hline \hline
 \rule{0mm}{4mm} 3d NFFT & 62.81 & 94.15 & 188.35 & 345.39 & \\[1ex] \hline
 \rule{0mm}{4mm} 2d$\otimes$1d NFFT & 144.36 & 216.62 & 432.82 &  & \\[1ex] \hline
...
In the second example we test our algorithms with MR measurements of a physical phantom by 
the Philips Achieva 1.5T device.

\marginpar{fieldmap???}
Wie wurde die fieldmap berechnet? siehe Fessler?

The field map is shown in Figure \ref{fieldmap}. The off-resonance is in the interval $[-??,???]$ kHz, which 
causes ??? $\pi$ extra spin phase accural during the readout.

\begin{figure}[ht!] 
\begin{center}
\includegraphics[width=6cm]{pics/fieldmap.jpg} 
\end{center}
