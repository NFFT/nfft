                  ...
                  double mu, double c, char *fout_name) 
{
  ...
  nfct_plan  my_cplan, my_other_cplan;        /* plans for the two dimensional nfct */
  infct_plan my_icplan;                       /* plan for the two dimensional infct */
  ...
  nfct_init_2d(&my_cplan, my_N, my_N, M);

  /* initialise my_icplan, specific */
  infct_init_advanced( &my_icplan, &my_cplan, CGNE | PRECOMPUTE_DAMP);
  ...
  /* precompute psi */
  if(my_cplan.nfct_flags & PRE_PSI)
    nfct_precompute_psi( &my_cplan);


  /** initialise damping factors */
  if(my_icplan.flags & PRECOMPUTE_DAMP)
    {
      for(k0=0;k0<my_cplan.N[0];k0++)
        for(k1=0;k1<my_cplan.N[1];k1++)
        {
          my_icplan.w_hat[k0*my_cplan.N[1]+k1]=
            modified_multiquadric( mu, c, norm2(k0, k1));
        }
      
//      for(k0=0;k0<my_cplan.N[0];k0++)
//        my_icplan.w_hat[k0*my_cplan.N[1]+my_cplan.N[1]-1]=0;
//      for(k1=0;k1<my_cplan.N[1];k1++)
//        my_icplan.w_hat[(my_cplan.N[0]-1)*my_cplan.N[1]+k1]=0;    
    }
  ...
  /* inverse trafo */  
  infct_before_loop( &my_icplan);
  for(l=0; l < iter; l++)
  { 
      infct_loop_one_step( &my_icplan);
      //fprintf( stderr, "%e\n", my_icplan.dot_r_iter);
  }



  nfct_init_2d( &my_other_cplan, my_N, my_N, my_M * my_M);
  ...
  /* precompute psi */
  if( my_other_cplan.nfct_flags & PRE_PSI)
    nfct_precompute_psi( &my_other_cplan);

  NFFT_SWAP_double( my_icplan.f_hat_iter, my_other_cplan.f_hat);
  nfct_trafo( &my_other_cplan);
  NFFT_SWAP_double( my_icplan.f_hat_iter, my_other_cplan.f_hat);
  ...
  infct_finalize( &my_icplan);  
  nfct_finalize( &my_cplan);
  nfct_finalize( &my_other_cplan);
}
