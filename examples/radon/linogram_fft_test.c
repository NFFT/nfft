/*gcc -o linogram_fft_test linogram_fft_test.c -lnfft3 -lfftw3 -lm -I/home/mfenn/NFFT3_develop/lib/trunk/include -L/home/mfenn/NFFT3_develop/lib/trunk/.libs -L/usr/local/lib*/
#include <math.h>
#include <stdlib.h>
#include "util.h"
#include "nfft3.h"
...
int linogram_fft(fftw_complex *f_hat, int NN, fftw_complex *f, int T, int R)
{
...
int inverse_linogram_fft(fftw_complex *f, int T, int R, fftw_complex *f_hat, int NN, int max_i)
{
  int j,k;                              /**< index for nodes and freqencies   */
  nfft_plan my_nfft_plan;               /**< plan for the nfft-2D             */
...
int main(int argc,char **argv)
{
  ...
  if( argc!=5 )
  {
    printf("linogram_fft_test N T R max_i\n");
    printf("\n");
  ...
  T = atoi(argv[2]);
  R = atoi(argv[3]);
  printf("N=%d, linogram grid with T=%d, R=%d. \n",N,T,R);
  max_i = atoi(argv[4]);
  ...
  /** linogram FFT */
  linogram_fft(f_hat,N,f,T,R);

  /** inverse linogram FFT */
  inverse_linogram_fft(f,T,R,f_hat2,N,max_i);
  ...
  /** write result */
  fp=fopen("linogram_fft_data.dat","w+");
  if (fp==NULL)
    return(-1);
  for(k=0;k<N*N;k++)
  {
