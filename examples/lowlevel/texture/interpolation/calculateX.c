...
void write_grid_h(const char *file)
{
	int i;
	FILE *f = fopen(file, "w");

	fprintf(f, "Polefigures\n");
	fprintf(f, "# This file was automatically generated by calculateX.\n");
...
void write_grid_r(const char *file)
{
	int j;
	FILE *f = fopen(file, "w");
...
void create_grid()
{
	if (grid_type == 0) {
		...
		write_grid_h(grid_h_file);
		write_grid_r(grid_r_file);
	} else {
