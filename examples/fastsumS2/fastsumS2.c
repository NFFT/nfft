         ...
         printf("\tt_f = %f",t_f);
          ...
          /*for (k = 0; k < ld[ild][1]; k++)
          {
              fprintf(stderr,"f[%d] = %le + I*%le \tf_m[%d] = %le + I*%le\n",
                k,creal(f[k]),cimag(f[k]),k,creal(f_m[k]),cimag(f_m[k]));
					...
					if (ld[ild][2] != NO)
					{
            //err = l_infty_complex(f, f_m, ld[ild][1])/l_1_complex(b,NULL,ld[ild][0]);
            err = error_l_infty_1_complex(f, f_m, ld[ild][1], b, ld[ild][0]);
            printf("\terr = %le\n",err);
					}
					else
					{
          ...
          if (ld[ild][2] != 0)
          {
            if (kt == KT_ABEL_POISSON || kt == KT_SINGULARITY || kt == KT_GAUSSIAN)
            {
              ...
              ld[ild][0],ld[ild][1],t_d,t_f,error_l_infty_1_complex(f, f_m, 
              ld[ild][1], b, ld[ild][0]));
            }
            else if (kt == KT_LOC_SUPP)
            {
              fprintf(file_dat,"%.2f %2f %3d %6d %6d %5.2f %5.2f %.4E\n",p[ip][0],
              p[ip][1],m[im],ld[ild][0],ld[ild][1],t_d,t_f,error_l_infty_1_complex(f, 
              f_m, ld[ild][1], b, ld[ild][0]));
            }
          }
          else
          {
