...
struct octreenode{
    ...
    int sas; /*Zuordnung ob Knoten zum SAS-Volume(ses=1) zum SES-Surface(sas=2) oder au�erhalb des Molek�ls liegt (sas=0)*/
...
int main(int argc, char *argv[ ])
{
   ...
   /*Speicherplatz f�r die Atomkoordinaten fuer Molekuel A und B*/
    ...
    */
    ...
    root = (struct octreenode *) malloc(sizeof (struct octreenode));
...
struct octreenode* refine_tree(struct octreenode *this,double c_a[3], double l)
{
  ...
  if(laenge>0.5)
  {
     for(i=0;i<8;i++)
     {
        ...
        this->child[i] = (struct octreenode *) malloc(sizeof (struct octreenode));
  ...
  return this;
    ...
    void traverse_tree(struct octreenode *this, double *atomkoordinaten, double atomradius[], int molekuelzahl)
    {
     ...
     this -> sas=0; /*Initialisierung Zelle liegt au�erhalb des Molekuels*/
     ...
     if(vertices_sum<=7) /*wenn vertices_sum kleiner 7 und groesser 0 ist gehoert dieser w�rfel zum SAS Surface
                           in diese
