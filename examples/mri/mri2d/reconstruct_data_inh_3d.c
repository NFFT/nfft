...
void nfft (char* filename,int N,int M,int iteration , int weight)
{
  ...
  imri_inh_3d_init_advanced(&my_iplan,&my_plan, infft_flags );
  ...
  /* inverse trafo */
  imri_inh_3d_before_loop(&my_iplan);
  for(l=0;l<iteration;l++)
  {
    ...
    l+1,iteration);
    imri_inh_3d_loop_one_step(&my_iplan);
  }
