...
AC_ARG_ENABLE(add-to-cflags, [AC_HELP_STRING([--enable--add-to-cflags], [Add guessed compiler flags to CFLAGS])], 
	acx_maxopt_addcf=$enableval, acx_maxopt_addcf=no)

if test "x$acx_maxopt_addcf" != "xno"; then
  maxopt_save_CFLAGS=$CFLAGS
  ac_test_CFLAGS=""
  echo "Saved CFLAGS $maxopt_save_CFLAGS in maxopt_save_CFLAGS"
fi
...
if test "$ac_test_CFLAGS" != "set"; then
  ...
  if test "$acx_maxopt_addcf" = "yes"; then
    CFLAGS="$maxopt_save_CFLAGS $CFLAGS"
    echo "Guessed and saved CFLAGS are $CFLAGS"
  fi
