                      ...
                      unsigned fftw_flags, unsigned planning)
{
  ...
  for (dirn = FWD; dirn <= ADJ; dirn++) {
    ...
    Y(problem_md5)(pl, p->prob[dirn], sig);
    q.l = F;
    q.u = F;
    q.timelimit_imp = 0;
    q.info = 0;
   ...
   * winner's psi is built once and Y(precompute) only upgrades it to AWAKE. */
  {
    ...
    Y(
    nfft_x_md5)(((const problem_nfft *)p->prob[FWD])->x, dM_guard, x_sig_entry);
# define VERIFY_X                                                             \
...
# define VERIFY_X ((void)0)
    ...
    for (dirn = FWD; dirn <= ADJ; dirn++) {
      ...
      for (ci = 0; ci < nc; ci++) {
        ...
        Y(plan_awake)(cands[dirn][ci], PLNR_AWAKE_ZERO);
        ...
        if (winner_idx < 0 || cost < winner_cost) {
          if (winner_idx >= 0) {
            Y(plan_awake)(cands[dirn][winner_idx], PLNR_SLEEPY);
            VERIFY_X;
          }
          winner_idx = ci;
          winner_cost = cost;
        } else {
          Y(plan_awake)(cands[dirn][ci], PLNR_SLEEPY);
          VERIFY_X;
        }
      }
    ...
    if (need_restart) {
      ...
      for (dj = FWD; dj <= ADJ; dj++) {
        ...
        for (cj = 0; cj < nc_dj; cj++)
          Y(plan_destroy)(cands[dj][cj]);
      }
      if (p->dir[FWD]) {
        Y(plan_destroy)(p->dir[FWD]);
        p->dir[FWD] = 0;
      }
      if (p->dir[ADJ]) {
        Y(plan_destroy)(p->dir[ADJ]);
        p->dir[ADJ] = 0;
      }
...
void Y(precompute)(Y(plan_ng) *p)
{
  ...
  for (i = 0; i < 2; ++i) {
    if (p->dir[i])
      Y(plan_awake)(p->dir[i], PLNR_AWAKE);
  }
}
...
void Y(execute_adjoint_on)(Y(plan_ng) *p, FC *f_hat, FC *f)
{
  problem_nfft *pf;
  C *save_pf_hat, *save_pf;
  A(p != 0);
