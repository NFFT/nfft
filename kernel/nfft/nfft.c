...
#define MACRO_without_PRE_PSI  PHI(ths->n[t2], \
  NX_SUB(ths->n[t2], ths->x[j*ths->d+t2], lj[t2]+u[t2])/((R)ths->n[t2]), t2)
...
static WINDOW_POLY_INLINE void B_serial_ ## which_one (X(plan) *ths, \
...
\
 ...
 \
    ...
    { \
      fg_e[t2] = EXP(K(-1.0) / ths->b[t2]); \
      fg_q[t2] = fg_e[t2] * fg_e[t2]; \
    } \
    for (k = 0; k < ths->M_total; k++) \
    { \
      INT j = (ths->flags & NFFT_SORT_NODES) ? ths->index_x[2*k+1] : k; \
 ...
 \
      ...
      { \
        ...
        const R fg_d = \
            NX_SUB(ths->n[t2], ths->x[j*ths->d+t2], FLOOR(fg_nx + K(0.5))); \
 ...
 \
      for (t2 = 0; t2 < ths->d; t2++) \
      { \
        y[t2] = ((NX_SUB(ths->n[t2], ths->x[j * ths->d + t2], u[t2])) \
          * ((R)(ths->K))) / (R)(ths->m + 2); \
...
static WINDOW_POLY_INLINE void B_openmp_A(X(plan) *ths,
    const int window_poly_on)
{
  INT lprod; /* 'regular bandwidth' of matrix B  */
  INT k;
...
{
#ifdef _OPENMP
  WINDOW_POLY_DISPATCH(B_openmp_A, ths);
#else
  WINDOW_POLY_DISPATCH(B_serial_A, ths);
#endif
}
...
static WINDOW_POLY_INLINE void B_openmp_T(X(plan) *ths,
    const int window_poly_on)
{
  INT lprod; /* 'regular bandwidth' of matrix B  */
  INT k;
...
{
#ifdef _OPENMP
  WINDOW_POLY_DISPATCH(B_openmp_T, ths);
#else
  WINDOW_POLY_DISPATCH(B_serial_T, ths);
#endif
}

/* ## specialized version for d=1  ########################################### */

/* e = exp(-1/b) and q = e * e, the outward step factors FG_RUN needs. */
static void nfft_init_fg(R *e, R *q, const R b)
{
  *e = EXP(K(-1.0)/b);
  *q = (*e) * (*e);
}
...
static WINDOW_POLY_INLINE void nfft_trafo_1d_B(X(plan) *ths,
    const int window_poly_on)
{
  ...
  if (ths->flags & FG_PSI)
  {
    ...
    for (k = 0; k < M; k++)
    {
...

      {
        ...
        const R fg_d =
            NX_SUB(n, ths->x[j], FLOOR(fg_nx + K(0.5)));
        FG_RUN(psij_const, m,
  ...
  if (ths->flags & PRE_LIN_PSI)
  {
    ...
    for (k = 0; k < M; k++)
    {
      ...
      ip_y = FABS(NX_SUB(n, ths->x[j], u)) * ((R)ip_s);
...
{ \
 ...
 \
            uo(ths, j, &u, &o, (INT)0); \
 \
            ip_y = FABS(NX_SUB(((R)n), ths->x[j], u)) * ((R)ip_s); \
...
static WINDOW_POLY_INLINE void nfft_adjoint_1d_B(X(plan) *ths,
    const int window_poly_on)
{
...
void X(trafo_1d)(X(plan) *ths)
{
...

  {
    ...
    TIC(2);
    WINDOW_POLY_DISPATCH(nfft_trafo_1d_B, ths);
    TOC(2);
  }
}
...
void X(adjoint_1d)(X(plan) *ths)
{
  ...
  TIC(2)
  WINDOW_POLY_DISPATCH(nfft_adjoint_1d_B, ths);
  TOC(2)
...
static WINDOW_POLY_INLINE void nfft_trafo_2d_B(X(plan) *ths,
    const int window_poly_on)
{
  const C *g = (C*)ths->g;
  ...
  if(ths->flags & PRE_FG_PSI)
  {
    ...
    nfft_init_fg(&fg_e[0], &fg_q[0], ths->b[0]);
    nfft_init_fg(&fg_e[1], &fg_q[1], ths->b[1]);
  ...
  if(ths->flags & FG_PSI)
  {
    ...
    for (k = 0; k < M; k++)
    {
...

      {
        ...
        const R fg_d =
            NX_SUB(n0, ths->x[2*j], FLOOR(fg_nx + K(0.5)));
        FG_RUN(psij_const, m,
...

      {
        ...
        const R fg_d =
            NX_SUB(n1, ths->x[2*j+1], FLOOR(fg_nx + K(0.5)));
  ...
  if(ths->flags & PRE_LIN_PSI)
  {
    ...
    for (k = 0; k < M; k++)
    {
      ...
      uo(ths,j,&u,&o,(INT)0);
      ip_y = FABS(NX_SUB(n0, ths->x[2*j], u)) * ((R)ip_s);
      ip_u = (INT)LRINT(FLOOR(ip_y));
      ip_w = ip_y - (R)(ip_u);
      for (l = 0; l < 2*m+2; l++)
...
{ \
 ...
 \
            uo(ths,j,&u,&o,(INT)0); \
            ip_y = FABS(NX_SUB(((R)n0), (ths->x[2*j]), u))*((R)ip_s); \
            ip_u = LRINT(FLOOR(ip_y)); \
            ip_w = ip_y-ip_u; \
            for(l=0; l < 2*m+2; l++) \
              psij_const[l] = ths->psi[ABS(ip_u-l*ip_s)]*(K(1.0)-ip_w) + \
...
static WINDOW_POLY_INLINE void nfft_adjoint_2d_B(X(plan) *ths,
    const int window_poly_on)
{
  const INT n0 = ths->n[0];
  const INT n1 = ths->n[1];
...
void X(trafo_2d)(X(plan) *ths)
{
  ...
  TIC(2);
  WINDOW_POLY_DISPATCH(nfft_trafo_2d_B, ths);
  TOC(2);
}
...
void X(adjoint_2d)(X(plan) *ths)
{
  ...
  TIC(2);
  WINDOW_POLY_DISPATCH(nfft_adjoint_2d_B, ths);
  TOC(2);
...
static WINDOW_POLY_INLINE void nfft_trafo_3d_B(X(plan) *ths,
    const int window_poly_on)
{
  const INT n0 = ths->n[0];
  const INT n1 = ths->n[1];
  ...
  if(ths->flags & PRE_FG_PSI)
  {
    ...
    nfft_init_fg(&fg_e[0], &fg_q[0], ths->b[0]);
    nfft_init_fg(&fg_e[1], &fg_q[1], ths->b[1]);
    nfft_init_fg(&fg_e[2], &fg_q[2], ths->b[2]);
  ...
  if(ths->flags & FG_PSI)
  {
    ...
    for (k = 0; k < M; k++)
    {
...

      {
        ...
        const R fg_d =
            NX_SUB(n0, ths->x[3*j], FLOOR(fg_nx + K(0.5)));
        FG_RUN(psij_const, m,
            (PHI(ths->n[0], fg_d / (R)(n0), 0)),
            EXP(K(2.0) * fg_d / ths->b[0]),
            fg_e[0], fg_q[0]);
...

      {
        ...
        const R fg_d =
            NX_SUB(n1, ths->x[3*j+1], FLOOR(fg_nx + K(0.5)));
        FG_RUN(psij_const + 2*m+3, m,
            (PHI(ths->n[1], fg_d / (R)(n1), 1)),
            EXP(K(2.0) * fg_d / ths->b[1]),
            fg_e[1], fg_q[1]);
...

      {
        ...
        const R fg_d =
            NX_SUB(n2, ths->x[3*j+2], FLOOR(fg_nx + K(0.5)));
  ...
  if(ths->flags & PRE_LIN_PSI)
  {
    ...
    for (k = 0; k < M; k++)
    {
      ...
      uo(ths,j,&u,&o,(INT)0);
      ip_y = FABS(NX_SUB(n0, ths->x[3*j+0], u)) * ((R)ip_s);
      ip_u = (INT)(LRINT(FLOOR(ip_y)));
      ip_w = ip_y - (R)(ip_u);
      for(l=0; l < 2*m+2; l++)
        psij_const[l] = ths->psi[ABS(ip_u-l*ip_s)]*(K(1.0)-ip_w) +
...
{ \
 ...
 \
            uo(ths,j,&u,&o,(INT)0); \
            ip_y = FABS(NX_SUB(((R)n0), ths->x[3*j+0], u))*((R)ip_s); \
            ip_u = LRINT(FLOOR(ip_y)); \
            ip_w = ip_y-ip_u; \
            for(l=0; l < 2*m+2; l++) \
              psij_const[l] = ths->psi[ABS(ip_u-l*ip_s)]*(K(1.0)-ip_w) + \
...
static WINDOW_POLY_INLINE void nfft_adjoint_3d_B(X(plan) *ths,
    const int window_poly_on)
{
  INT k;
  const INT n0 = ths->n[0];
...
void X(trafo_3d)(X(plan) *ths)
{
  ...
  TIC(2);
  WINDOW_POLY_DISPATCH(nfft_trafo_3d_B, ths);
  TOC(2);
}
...
void X(adjoint_3d)(X(plan) *ths)
{
  ...
  TIC(2);
  WINDOW_POLY_DISPATCH(nfft_adjoint_3d_B, ths);
  TOC(2);
    ...
    for (j = 0; j < ths->M_total; j++)
      {
        ...
        const R d = NX_SUB(ths->n[t], ths->x[j*ths->d+t], FLOOR(nx + K(0.5)));
...
static WINDOW_POLY_INLINE void precompute_psi(X(plan) *ths,
    const int window_poly_on)
{
  INT t; /* index over all dimensions */
...
{
  WINDOW_POLY_DISPATCH(precompute_psi, ths);
}

#ifdef _OPENMP
static WINDOW_POLY_INLINE void nfft_precompute_full_psi_omp(X(plan) *ths,
    const int window_poly_on)
{
  INT j;                                /**< index over all nodes            */
...
static WINDOW_POLY_INLINE void precompute_full_psi(X(plan) *ths,
    const int window_poly_on)
{
#ifdef _OPENMP
  sort(ths);

  nfft_precompute_full_psi_omp(ths, window_poly_on);
#else
  INT t, t2; /* index over all dimensions */
  INT j; /* index over all nodes */
  INT l_L; /* plain index 0 <= l_L < lprod */
...
{
  WINDOW_POLY_DISPATCH(precompute_full_psi, ths);
}
