...
int Y(tensor_equal)(const tensor *a, const tensor *b)
{
  ...
  for (i = 0; i < a->rnk; ++i) {
    ...
    if (x->n_in != y->n_in || x->is != y->is || x->n_out != y->n_out
        || x->os != y->os)
      return 0;
  }
  return 1;
...
{
  int i;
  Y(md5_put_int)(ctx, t->rnk);
  for (i = 0; i < t->rnk; ++i) {
    Y(md5_put_INT)(ctx, t->dims[i].n_in);
    Y(md5_put_INT)(ctx, t->dims[i].is);
    Y(md5_put_INT)(ctx, t->dims[i].n_out);
    Y(md5_put_INT)(ctx, t->dims[i].os);
  }
}
