...
#define MD5_FF(a, b, c, d, x, s, t)                                           \
  ((a) = MD5_M32(                                                             \
        (b) + MD5_ROT(MD5_M32((a) + MD5_F((b), (c), (d)) + (x) + (t)), (s))))
#define MD5_GG(a, b, c, d, x, s, t)                                           \
  ((a) = MD5_M32(                                                             \
        (b) + MD5_ROT(MD5_M32((a) + MD5_G((b), (c), (d)) + (x) + (t)), (s))))
#define MD5_HH(a, b, c, d, x, s, t)                                           \
  ((a) = MD5_M32(                                                             \
        (b) + MD5_ROT(MD5_M32((a) + MD5_H((b), (c), (d)) + (x) + (t)), (s))))
#define MD5_II(a, b, c, d, x, s, t)                                           \
  ((a) = MD5_M32(                                                             \
        (b) + MD5_ROT(MD5_M32((a) + MD5_I((b), (c), (d)) + (x) + (t)), (s))))
...
static const md5uint T64[64] = {
     ...
     0x49b40821UL,
     /* Round 2 */
     0xf61e2562UL, 0xc040b340UL, 0x265e5a51UL, 0xe9b6c7aaUL, 0xd62f105dUL,
     0x02441453UL, 0xd8a1e681UL, 0xe7d3fbc8UL, 0x21e1cde6UL, 0xc33707d6UL,
     0xf4d50d87UL, 0x455a14edUL, 0xa9e3e905UL, 0xfcefa3f8UL, 0x676f02d9UL,
     0x8d2a4c8aUL,
     /* Round 3 */
     0xfffa3942UL, 0x8771f681UL, 0x6d9d6122UL, 0xfde5380cUL, 0xa4beea44UL,
     0x4bdecfa9UL, 0xf6bb4b60UL, 0xbebfbc70UL, 0x289b7ec6UL, 0xeaa127faUL,
     0xd4ef3085UL, 0x04881d05UL, 0xd9d4d039UL, 0xe6db99e5UL, 0x1fa27cf8UL,
     0xc4ac5665UL,
     /* Round 4 */
     0xf4292244UL, 0x432aff97UL, 0xab9423a7UL, 0xfc93a039UL, 0x655b59c3UL,
     0x8f0ccc92UL, 0xffeff47dUL, 0x85845dd1UL, 0x6fa87e4fUL, 0xfe2ce6e0UL,
     0xa3014314UL, 0x4e0811a1UL, 0xf7537e82UL, 0xbd3af235UL, 0x2ad7d2bbUL,
     0xeb86d391UL};
...
static void compress(md5 *ctx)
{
  ...
  for (i = 0; i < 16; i++)
    x[i] = (md5uint)ctx->buf[4 * i] | ((md5uint)ctx->buf[4 * i + 1] << 8)
           | ((md5uint)ctx->buf[4 * i + 2] << 16)
           | ((md5uint)ctx->buf[4 * i + 3] << 24);
...
{
  Y(md5_put_bytes)(ctx, &i, sizeof(i));
}
...
{
  Y(md5_put_bytes)(ctx, &u, sizeof(u));
}
...
void Y(md5_end)(md5 *ctx)
{
  ...
  /* Append bit '1'. */
  Y(md5_put_byte)(ctx, (unsigned char)0x80);

  /* Pad to 56 (mod 64), leaving 8 bytes for the length. */
  while (ctx->len % 64 != 56)
    Y(md5_put_byte)(ctx, (unsigned char)0x00);

  bits_lo = save_len * 8u;
  Y(md5_put_byte)(ctx, (unsigned char)(bits_lo & 0xffu));
  Y(md5_put_byte)(ctx, (unsigned char)((bits_lo >> 8) & 0xffu));
  Y(md5_put_byte)(ctx, (unsigned char)((bits_lo >> 16) & 0xffu));
  Y(md5_put_byte)(ctx, (unsigned char)((bits_lo >> 24) & 0xffu));
  Y(md5_put_byte)(ctx, (unsigned char)0x00);
  Y(md5_put_byte)(ctx, (unsigned char)0x00);
  Y(md5_put_byte)(ctx, (unsigned char)0x00);
  Y(md5_put_byte)(ctx, (unsigned char)0x00);
