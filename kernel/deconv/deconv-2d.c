...
/* 2D DECONV solver: step A of the fast NFFT decomposition. Forward divides f_hat
...
static void run(const deconv_2d_plan *pln, const problem_deconv *pd,
                int forward)
{
  const INT N1 = pln->N1, n0 = pln->n0, n1 = pln->n1;
  INT len0[2], sof0[2],
       gof0[2]; /* run length, f_hat slot offset, grid offset */
...
{
  run((const deconv_2d_plan *)ego_, (const problem_deconv *)p, 1);
}

static void apply_adjoint(const plan *ego_, const problem *p)
{
  run((const deconv_2d_plan *)ego_, (const problem_deconv *)p, 0);
}
...
static const plan_adt deconv_2d_plan_adt = {apply, awake, print, destroy,
                                            apply_adjoint};
...
static plan *mkplan_deconv_2d(const solver *ego, const problem *p, planner *pl)
{
  ...
  pln = (deconv_2d_plan *)Y(plan_create)(sizeof(deconv_2d_plan),
                                      &deconv_2d_plan_adt);
...
static const solver_adt deconv_2d_adt = {NFFT_PROBLEM_DECONV, 0,
                                         mkplan_deconv_2d};
