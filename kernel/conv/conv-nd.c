...
static void fill_u(conv_nd_plan *pln)
{
...
static void fill_psi(conv_nd_plan *pln)
{
  const int d = pln->d, m = pln->m;
  int t;
  for (t = 0; t < d; t++)
    Y(window_phi_precompute)(pln->window, pln->n[t], pln->N[t], m, pln->x + t, d,
                          pln->M, pln->psi + t * (2 * m + 2), d * (2 * m + 2));
}
