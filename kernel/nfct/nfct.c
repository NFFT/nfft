...
#define MACRO_ndct_init_result_transposed  \
...
lb[(act_dim)] = (int)( NODE((j),(act_dim)) * nfct_fftw_2N( ths->n[(act_dim)])) - ths->m;    \
}
...
if( lb[(i0)] < 0)							\
lg_offset[(i0)] = (lb[(i0)] % nfct_fftw_2N( ths->n[(i0)])) + nfct_fftw_2N( ths->n[(i0)]);\
else								        \
