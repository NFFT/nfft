 ...
 * regenerated with Mathematica from file lambda.nb. */
  ...
  static const long double lanzcos[5] =
  {
...
#else
  static const long double lanzcos[10] =
  {
    K(0.0005636587928273201791569281430152314357672980871136164593992784701),
    K(1.7047720038921317519705470019743083380768612897684016290166703774895),
    K(-4.0441933735243976458251401848223374517700933008218455416563834632622),
    K(3.4702744974355393826681043881782281178470503189202023161633612982314),
    K(-1.3107031496648250400492701956635601000596751456149415305010986814809),
    K(0.211472705419326808127615189152662074883303866384207413813469209953),
    K(-0.0118956476655966776861650927966311308539666218545113772684935105452),
    K(0.0001354103458427681864227596903130239651460734463143140786675192983),
    K(-4.98088049754215910504944359312753058128179641799493621718506e-8),
    K(-1.30350879480064113791542355437005477749614975768299083612739121035e-12)
  };
  static const R g = K(8.4000000000000000000000000000000000000000000000000);
#endif

/* Computes the partial fractions sum in Lanzcos' approximation. */
static inline R csum(const R z)
{
  static const short n = sizeof(lanzcos)/sizeof(lanzcos[0]);
  R r = K(0.0), y = z + (R)(n-1);
  short i;
...
/* Computes lambda(z,eps) = Gamma(z+eps)/Gamma(z+1) using Lanczos' or Fields'
 * approximation. */
R nfft_lambda(const R z, const R eps)
{
  if (z + eps > K(20.0))
  {
    ...
    return EXP(-LOG1P(d/(zpg+emh))*(z+emh) + d*(K(1.0)-LOG(zpg+K(0.5))))
      * (csum(z-d)/csum(z));
  }
  else
    return EXP(LGAMMA(z+eps)-LGAMMA(z+K(1.0)));
}
