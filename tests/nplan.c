...
void Y(check_nplan_problem)(void)
{
  ...
  Y(problem_md5)(
       pl, p1, s1);
  p2 = Y(
       mkproblem_nfft)(1, &N1, 0, &n1, (INT)1023, 6, NFFT_WINDOW_KAISER_BESSEL,
                      1, 0u, 0, 0, 0, 0);
  ...
  p2 = Y(
       mkproblem_nfft)(1, &N1, 0, &n1, (INT)1024, 6, NFFT_WINDOW_KAISER_BESSEL,
                      1, 0u, 0, 0, 0, 0);
  Y(problem_md5)(pl, p2, s2);
  CU_ASSERT(s1[0] != s2[0] || s1[1] != s2[1] || s1[2] != s2[2]
            || s1[3] != s2[3]);
  ...
  {
    INT N2 = 32;
    p2 = Y(mkproblem_nfft)(1, &N2, 0, &n1, (INT)1000, 6,
                        NFFT_WINDOW_KAISER_BESSEL, 1, 0u, 0, 0, 0, 0);
    Y(problem_md5)(pl, p2, s2);
    CU_ASSERT(s1[0] != s2[0] || s1[1] != s2[1] || s1[2] != s2[2]
              || s1[3] != s2[3]);
    Y(problem_destroy)(p2);
  ...
  p2 = Y(
       mkproblem_nfft)(1, &N1, 0, &n1, (INT)1000, 8, NFFT_WINDOW_KAISER_BESSEL,
                      1, 0u, 0, 0, 0, 0);
  Y(problem_md5)(pl, p2, s2);
  CU_ASSERT(s1[0] != s2[0] || s1[1] != s2[1] || s1[2] != s2[2]
            || s1[3] != s2[3]);
   ...
   * only sign separates the directions' keys */
  {
    INT Ns = 16;
    problem *pf =
         Y(mkproblem_nfft)(1, &Ns, 0, &Ns, (INT)1000, 6,
                                 NFFT_WINDOW_KAISER_BESSEL, 1, 0u, 0, 0, 0, 0);
    problem *pa = Y(mkproblem_nfft)(1, &Ns, 0, &Ns, (INT)1000, 6,
                                 NFFT_WINDOW_KAISER_BESSEL, -1, 0u, 0, 0, 0,
                                 0);
    md5sig sf, sa;
    Y(problem_md5)(pl, pf, sf);
    Y(problem_md5)(pl, pa, sa);
    CU_ASSERT(sf[0] != sa[0] || sf[1] != sa[1] || sf[2] != sa[2]
              || sf[3] != sa[3]);
    Y(problem_destroy)(pf);
    Y(problem_destroy)(pa);
  }

  Y(problem_destroy)(p1);
...
void Y(check_nplan_elides_unit_axes_geometry)(void)
{
   ...
   * exactly the lower-rank problem, so the two hashes coincide. */
  {
    ...
    p = Y(
         mkproblem_nfft)(3, N3, 0, n3, (INT)100, 6, NFFT_WINDOW_KAISER_BESSEL, 1,
                       0u, 0, 0, 0, 0);
    CU_ASSERT_EQUAL(((const problem_nfft *)p)->sz->rnk,
                    3); /* borrowed: full rank kept */
    CU_ASSERT_EQUAL(Y(problem_nfft_N)(p, 2), (INT)1); /* unit axis visible */
    Y(problem_md5)(pl, p, s1);
    p2 = Y(
         mkproblem_nfft)(2, N2, 0, n2, (INT)100, 6, NFFT_WINDOW_KAISER_BESSEL,
                        1, 0u, 0, 0, 0, 0);
    Y(problem_md5)(pl, p2, s2);
    CU_ASSERT(s1[0] != s2[0] || s1[1] != s2[1] || s1[2] != s2[2]
              || s1[3] != s2[3]);
    ...
    p3 = Y(
         mkproblem_nfft)(3, N3, 0, n3, (INT)100, 6, NFFT_WINDOW_KAISER_BESSEL,
                        1, 0u, x3, 1, 0, 0);
    CU_ASSERT_EQUAL(((const problem_nfft *)p3)->sz->rnk,
                    2); /* top-level: compressed */
    Y(problem_md5)(pl, p3, s3);
    CU_ASSERT(s3[0] == s2[0] && s3[1] == s2[1] && s3[2] == s2[2]
              && s3[3] == s2[3]);

    Y(problem_destroy)(p);
    Y(problem_destroy)(p2);
    Y(problem_destroy)(p3);
    Y(planner_destroy)(pl);
  }
   ...
   * yields distinct problems and hashes: the x-column pairing differs. */
  {
    ...
    md5sig s1, s2;
    p = Y(
         mkproblem_nfft)(2, Na, 0, na, (INT)100, 6, NFFT_WINDOW_KAISER_BESSEL, 1,
                       0u, 0, 0, 0, 0);
    p2 = Y(
         mkproblem_nfft)(2, Nb, 0, nb, (INT)100, 6, NFFT_WINDOW_KAISER_BESSEL,
                        1, 0u, 0, 0, 0, 0);
    Y(problem_md5)(pl, p, s1);
    Y(problem_md5)(pl, p2, s2);
    CU_ASSERT(s1[0] != s2[0] || s1[1] != s2[1] || s1[2] != s2[2]
...
void Y(check_nplan_variant_key)(void)
{
  ...
  problem *pi =
       Y(mkproblem_nfft)(1, &N1, &v_i, &n1, 1000, 6,
                               NFFT_WINDOW_KAISER_BESSEL, +1, 0u, &x, 0, &fh,
                               &f);
  problem *pii =
       Y(mkproblem_nfft)(1, &N1, &v_ii, &n1, 1000, 6,
                                NFFT_WINDOW_KAISER_BESSEL, +1, 0u, &x, 0, &fh,
                                &f);
  Y(problem_md5)(Y(the_planner)(), pi, k_i);
  Y(problem_md5)(Y(the_planner)(), pii, k_ii);
  CU_ASSERT(k_i[0] != k_ii[0] || k_i[1] != k_ii[1] || k_i[2] != k_ii[2]
            || k_i[3] != k_ii[3]);
  {
    problem *pn =
         Y(mkproblem_nfft)(1, &N1, 0, &n1, 1000, 6,
                                 NFFT_WINDOW_KAISER_BESSEL, +1, 0u, &x, 0, &fh,
                                 &f);
    md5sig kn;
    Y(problem_md5)(Y(the_planner)(), pn, kn);
    CU_ASSERT(kn[0] == k_i[0] && kn[1] == k_i[1] && kn[2] == k_i[2]
              && kn[3] == k_i[3]); /* NULL == explicit type-I */
    Y(problem_destroy)(pn);
  }
  CU_ASSERT_EQUAL(Y(problem_nfft_variant)(pii, 0), NFFT_NDFT_TYPE_II);
  problem *poi =
       Y(mkproblem_nfft)(1, &No, &v_i, &no, 1000, 6,
                                NFFT_WINDOW_KAISER_BESSEL, +1, 0u, &x, 0, &fh,
                                &f);
  problem *poii =
       Y(mkproblem_nfft)(1, &No, &v_ii, &no, 1000, 6,
                                 NFFT_WINDOW_KAISER_BESSEL, +1, 0u, &x, 0, &fh,
                                 &f);
  Y(problem_md5)(Y(the_planner)(), poi, ko_i);
  Y(problem_md5)(Y(the_planner)(), poii, ko_ii);
  CU_ASSERT(ko_i[0] == ko_ii[0] && ko_i[1] == ko_ii[1] && ko_i[2] == ko_ii[2]
            && ko_i[3] == ko_ii[3]); /* odd collapses */
  CU_ASSERT_EQUAL(Y(problem_nfft_variant)(poii, 0), NFFT_NDFT_TYPE_I);
  Y(problem_destroy)(pi);
  Y(problem_destroy)(pii);
  Y(problem_destroy)(poi);
...
void Y(check_nplan_key_is_data_blind)(void)
{
  ...
  p0 = Y(
       mkproblem_nfft)(1, &N, 0, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, 1, 0u, 0,
                      0, 0, 0);
  p1 = Y(
       mkproblem_nfft)(1, &N, 0, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, 1, 0u, x,
                      /*copy_x=*/1, f_hat, f);
  Y(problem_md5)(pl, p0, s0);
  Y(problem_md5)(pl, p1, s1);
  CU_ASSERT(s0[0] == s1[0] && s0[1] == s1[1] && s0[2] == s1[2]
            && s0[3] == s1[3]);

  Y(problem_destroy)(p0);
  Y(problem_destroy)(p1);
  Y(planner_destroy)(pl);
}
...
static problem *mk_problem(int d, INT Nv, INT nv, INT M, int m, int sign)
{
  ...
  return Y(mkproblem_nfft)(d, N, 0, n, M, m,
                           Y(get_window_id)(), sign, 0u, 0, 0, 0,
                        0);
}
...
void Y(check_nplan_guard_declines)(void)
{
  ...
  Y(planner_forget)(pl, PLNR_FORGET_ALL);
  {
     ...
     * reads past it. */
    p = Y(
         mkproblem_nfft)(1, &N, 0, &n, 200000, 6, NFFT_WINDOW_KAISER_BESSEL, +1,
                       0u, &x, 0, &fh, &f);
  ...
  }
  {
    ...
    p = Y(
         mkproblem_nfft)(1, &N, &v, &n, 200000, 6, NFFT_WINDOW_KAISER_BESSEL, +1,
                       0u, &x, 0, &fh, &f);
    expect_winner(pl, p, "nfft_solver_fast_native");
    Y(problem_destroy)(p);
  }
  ...
  /* NO_DIRECT + odd: the fast serves it. */
  {
    ...
    for (j = 0; j < M; j++)
      x[j] = K(0.0);
    pg = Y(
         plan_ng_guru)(1, &N, 0, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, x, fh, f,
                      0u, NFFT_ESTIMATE | NFFT_NO_DIRECT);
    CU_ASSERT_PTR_NOT_NULL(pg);
    Y(plan_ng_destroy)(pg);
    Y(free)(x);
    Y(free)(fh);
    Y(free)(f);
  }
...
void Y(check_nplan_ndft_multivariate_dispatch)(void)
{
  ...
  Y(problem_destroy)(p);
  p = mk_problem(2, 32, 64, 4096, 6,
                 -1); /* adjoint dispatches on its own sign */
  expect_winner(pl, p, "nfft_solver_ndft_nd");
  Y(problem_destroy)(p);
  p = mk_problem(3, 16, 32, 4096, 3, 1);
  expect_winner(pl, p, "nfft_solver_ndft_nd");
                                      ...
                                      unsigned seed, unsigned planning)
{
  ...
  p = Y(
       plan_ng_guru)(d, N, 0, n, M, 6, NFFT_WINDOW_KAISER_BESSEL, xin, f_hat, f,
                   0u, planning);
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
  Y(precompute)(p);
  ...
  /* legacy direct oracle on identical inputs */
  {
    ...
    NFFT(init_guru)(&ref, d, Ni, (int)M, ni, 6, MALLOC_X | MALLOC_F_HAT | MALLOC_F,
              0u);
  }
  for (j = 0; j < d * M; j++)
    ref.x[j] = xin[j];
...
static void ndft_ref_adjoint(int d, const INT *N, const int *variant, INT Ntot,
                             INT M, const R *x, const C *f, C *f_hat)
{
  INT j, kl;
  for (kl = 0; kl < Ntot; kl++) {
    INT tmp = kl, kt[d];
...
void Y(check_nplan_correct)(void)
{
   ...
   * x/f_hat/f are still required arguments, though never read here. */
  {
    ...
    CU_ASSERT_PTR_NULL(
         Y(plan_ng_guru)(1, &N, 0, &n, M, 6,
                                    NFFT_WINDOW_KAISER_BESSEL, x, f_hat, f, 0u,
                                    NFFT_ESTIMATE | NFFT_NO_DIRECT));
  }
...
void Y(check_nplan_measured)(void)
{
  ...
  if (Y(the_planner)()->htab_blessed.nelem == 0u) {
    ...
    CU_FAIL("exhaustive mode requires a working cycle counter; measured path "
            "unverifiable");
#else
    CU_PASS("no usable cycle counter: measured degraded to estimate-grade "
            "(known-skipped)");
#endif
   ...
   * bundle came out */
  {
    ...
    p = Y(plan_ng_guru)(1, &Nd, 0, &nd, Md, 6, NFFT_WINDOW_KAISER_BESSEL, xd,
                     f_hatd, fd, 0u, NFFT_MEASURE);
    CU_ASSERT_PTR_NOT_NULL_FATAL(p);
...
void Y(check_nplan_destructive_default)(void)
{
  ...
  Y(execute)(p);
  {
    ...
    NFFT(init_guru)(&ref, 1, &Ni, (int)M, &ni, 6, MALLOC_X | MALLOC_F_HAT | MALLOC_F,
              0u);
  }
  for (j = 0; j < M; j++)
    ref.x[j] = x[j];
...
void Y(check_nplan_execute_on)(void)
{
   ...
   * longer applies. */
  p = Y(plan_ng_guru)(1, &N, 0, &n, M, 6, Y(get_window_id)(), x, f_hat, f, 0u,
                   NFFT_ESTIMATE);
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
  Y(precompute)(p);
...
void Y(check_nplan_timelimit_tight_degrades_to_estimate)(void)
{
  ...
  Y(set_timelimit)(0.0); /* tight: every candidate race bails at once */
  p = Y(
       plan_ng_guru)(1, &N, 0, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, x, f_hat, f,
                   0u, NFFT_MEASURE);
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
  ...
  Y(set_timelimit)(-1.0); /* restore unlimited for the rest of the suite */
  Y(the_planner_destroy)();
}
...
static const plan_adt burn_plan_adt = {burn_apply, 0, burn_print, 0,
                                       burn_apply};
...
void Y(check_nplan_apply_adjoint)(void)
{
  ...
  p = Y(
       plan_ng_guru)(1, &N, 0, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, xin, f_hat,
                   f, 0u, NFFT_MEASURE);
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
  Y(precompute)(p);
...
/* The native NDFT holds up in the ill-conditioned regime (large N, nodes near
 * +-1/2), where a per-term direct sum loses to libm reducing huge phase
 * arguments. Both it and the legacy direct now run the same blocked recurrence
 * off Y(nfft_ndft_reduced_omega), so the native one must be no worse. The
 * reference sums in long double with argument reduction, independent of R. */
void Y(check_nplan_ndft_accuracy)(void)
{
...

  {
    ...
    NFFT(init_guru)(&lref, 1, &Ni, (int)Mm, &ni, 6,
...
void Y(check_nplan_direct_only_bundle)(void)
{
  ...
  /* (a) tiny M -> direct NDFT beats fast; still executes. */
  {
    ...
    {
      int Ni = (int)N, ni = (int)n;
      NFFT(init_guru)(&ref, 1, &Ni, (int)M, &ni, 6,
                MALLOC_X | MALLOC_F_HAT | MALLOC_F, 0u);
    }
    for (j = 0; j < M; j++)
      ref.x[j] = x[j];
  ...
  /* (c) a pure multivariate NDFT-only bundle. */
  {
    ...
    p = Y(
         plan_ng_guru)(2, N, 0, n, M, 6, NFFT_WINDOW_KAISER_BESSEL, x, f_hat, f,
                     0u, NFFT_ESTIMATE);
    CU_ASSERT_PTR_NOT_NULL_FATAL(p);
    ...
    Y(execute)(p);
    {
      ...
      NFFT(init_guru)(&ref, 2, Ni, (int)M, ni, 6, MALLOC_X | MALLOC_F_HAT | MALLOC_F,
                0u);
    }
    for (j = 0; j < 2 * M; j++)
      ref.x[j] = x[j];
...
void Y(check_nplan_variant_guru)(void)
{
  ...
  p = Y(plan_ng_guru)(1, &N, &variant, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, x,
                   f_hat, f, 0u, NFFT_ESTIMATE);
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
  Y(plan_ng_destroy)(p);
  Y(the_planner_destroy)();
  Y(free)(x);
...
static void type_ii_1d_once(unsigned steer)
{
  ...
  p = Y(plan_ng_guru)(1, &N, &v_ii, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, xin,
  ...
  ndft_ref_trafo(1, &N, 0, Ntot, M, xin, f_hat, ref_i);
  {
    ...
    CU_ASSERT(Y(test_rel_max_err)(ref, mod, M)
              < tol); /* f_II = e^{-2pi i x} f_I */
    Y(free)(mod);
  }
  CU_ASSERT(Y(test_rel_max_err)(ref, ref_i, M)
            > (R)1.0e-3); /* differs from type-I */
  for (j = 0; j < M; j++)
    f[j] = ref[j];
  Y(execute_adjoint)(p);
...
void Y(check_nplan_user_x_pristine)(void)
{
  ...
  Y(precompute)(p);
  Y(execute)(p); /* must still run on the copy, not the clobbered user x */
  Y(plan_ng_destroy)(p);
  Y(free)(x);
  Y(free)(snap);
  Y(free)(f_hat);
  Y(free)(f); /* user frees own x */
}
...
void Y(check_nplan_awake_zero_restore)(void)
{
  ...
  Y(plan_awake)(pl, PLNR_AWAKE_ZERO);
  {
    int any = 0;
  ...
  Y(plan_destroy)(pl);
  Y(problem_destroy)(pr); /* frees the copy */
  Y(free)(x); /* test still owns its own array */
}
...
void Y(check_nplan_restore_guard_fires)(void)
{
  ...
  Y(
  plan_awake)(pl, PLNR_AWAKE_ZERO); /* permute */
  Y(
  plan_awake)(pl, PLNR_SLEEPY); /* restore skipped */
  CU_ASSERT_FALSE(Y(nfft_x_verify)(((const problem_nfft *)pr)->x, M, sig));
  Y(
  nfft_perm_break_restore) = 0; /* undo for a clean teardown */
  Y(
  plan_awake)(pl, PLNR_AWAKE_ZERO);
  Y(
  plan_awake)(pl, PLNR_SLEEPY);
  Y(
  plan_destroy)(pl);
  Y(
  problem_destroy)(pr);
  ...
  free)(x);
#else
  CU_PASS("restore guard is A(...)-gated; build with ./configure "
          "--enable-debug");
#endif
}
...
void Y(check_nplan_measured_prunes_by_estimate)(void)
{
  ...
  p = Y(
       plan_ng_guru)(1, &N, 0, &n, M, 4, NFFT_WINDOW_KAISER_BESSEL, x, f_hat, f,
                   0u, NFFT_MEASURE);
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
...
void Y(check_nplan_in_race_guard_passes)(void)
{
  ...
  Y(nfft_ensure_registered)();
  Y(nfft_solver_perm_test_register)(Y(the_planner)());
...
void Y(check_nplan_awake_zero_internal)(void)
{
  ...
  p = Y(
       plan_ng_guru)(1, &N, 0, &n, M, 6, NFFT_WINDOW_KAISER_BESSEL, x, f_hat, f,
                   0u, NFFT_ESTIMATE);
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
  CU_ASSERT_EQUAL(Y(plan_ng_test_awake_state)(p),
                  PLNR_SLEEPY); /* winner SLEEPY */
  Y(precompute)(p);
   ...
   * (PLNR_PRUNE_RATIO) and two candidates actually race. */
  {
    ...
    p = Y(
         plan_ng_guru)(1, &Nm, 0, &nm, Mm, 6, NFFT_WINDOW_KAISER_BESSEL, xm, fhm,
                     fm, 0u, NFFT_MEASURE);
    CU_ASSERT_PTR_NOT_NULL_FATAL(p);
...
void Y(check_nplan_unit_axis_correct)(void)
{
  ...
  for (s = 0; s < 4; s++) {
    ...
    {
      Y(plan_ng) *pf =
           Y(plan_ng_guru)(d, N, 0, n, M, m, w, x, fh, f, 0u, NFFT_NO_DIRECT);
      CU_ASSERT_PTR_NOT_NULL(pf);
      if (pf)
        Y(plan_ng_destroy)(pf);
    }
    Y(plan_ng_destroy)(p);
    Y(free)(x);
    Y(free)(fh);
...
void Y(check_nplan_newarray_native_fast)(void)
{
  ...
  R *x = (R *)Y(malloc)((size_t)d * M * sizeof(R));
  C *fh = (C *)Y(
       malloc)((size_t)Ntot * sizeof(C)); /* plan-time bound arrays */
  C *f = (C *)Y(malloc)((size_t)M * sizeof(C));
...
void Y(check_nplan_unit_axis_execute_on)(void)
{
  ...
  CU_ASSERT_PTR_NOT_NULL_FATAL(p);
  Y(precompute)(p);
  Y(execute_on)(p, fh1, f); /* run on the new f_hat */
...
void Y(check_nplan_guru_rejects_null_args)(void)
{
  ...
  CU_ASSERT_PTR_NULL(Y(plan_ng_guru)(0, &N, 0, &n, M, 2,
                                  NFFT_WINDOW_KAISER_BESSEL, x, fh, f, 0u,
                                  NFFT_ESTIMATE)); /* d <= 0 */
  CU_ASSERT_PTR_NULL(
       Y(plan_ng_guru)(1, 0, 0, &n, M, 2, NFFT_WINDOW_KAISER_BESSEL,
                                  x, fh, f, 0u,
                                  NFFT_ESTIMATE)); /* N == NULL */
  CU_ASSERT_PTR_NULL(
       Y(plan_ng_guru)(1, &N, 0, 0, M, 2, NFFT_WINDOW_KAISER_BESSEL,
                                  x, fh, f, 0u,
                                  NFFT_ESTIMATE)); /* n == NULL */
  CU_ASSERT_PTR_NULL(Y(plan_ng_guru)(1, &N, 0, &n, M, 2,
                                  NFFT_WINDOW_KAISER_BESSEL, 0, fh, f, 0u,
                                  NFFT_ESTIMATE)); /* x == NULL */
  CU_ASSERT_PTR_NULL(Y(plan_ng_guru)(1, &N, 0, &n, M, 2,
                                  NFFT_WINDOW_KAISER_BESSEL, x, 0, f, 0u,
                                  NFFT_ESTIMATE)); /* f_hat == NULL */
  CU_ASSERT_PTR_NULL(Y(plan_ng_guru)(1, &N, 0, &n, M, 2,
                                  NFFT_WINDOW_KAISER_BESSEL, x, fh, 0, 0u,
                                  NFFT_ESTIMATE)); /* f == NULL */
...
void Y(check_nplan_guru_rejects_bad_geometry)(void)
{
   ...
   * surviving elision; NFFT_NO_FAST_NATIVE makes it legal again. */
  {
    ...
    pu = Y(
         plan_ng_guru)(2, Neq, 0, neq, M, 2, NFFT_WINDOW_KAISER_BESSEL, x, fh,
                      f, 0u, NFFT_ESTIMATE | NFFT_NO_FAST_NATIVE);
    CU_ASSERT_PTR_NOT_NULL(pu);
    Y(plan_ng_destroy)(pu);
    /* Unit axes are elided, so n[t] == N[t] == 1 stays legal. */
    pu = Y(
         plan_ng_guru)(2, Nun, 0, nun, M, 2, NFFT_WINDOW_KAISER_BESSEL, x, fh,
                      f, 0u, NFFT_ESTIMATE);
    CU_ASSERT_PTR_NOT_NULL(pu);
    Y(plan_ng_destroy)(pu);
  }
   ...
   * n > 2m + 2 at m = 15. NFFT_NO_FAST_NATIVE lifts it. */
  {
    ...
    CU_ASSERT_PTR_NULL(Y(plan_ng_guru)(1, &N1, 0, &n1, M, 15,
                                    ...
                                    NFFT_ESTIMATE));
    pu = Y(
         plan_ng_guru)(1, &N1, 0, &n1, M, 15, NFFT_WINDOW_KAISER_BESSEL, x, fh,
                      f, 0u, NFFT_ESTIMATE | NFFT_NO_FAST_NATIVE);
    CU_ASSERT_PTR_NOT_NULL(pu);
    Y(plan_ng_destroy)(pu);
  }
  ...
  /* NFFT_WINDOW_DIRAC_DELTA is a known ordinal, but no fast solver serves it. */
  {
    ...
    CU_ASSERT_PTR_NULL(Y(plan_ng_guru)(1, &N1, 0, &n1, M, 2,
                                    ...
                                    NFFT_ESTIMATE));
    pu = Y(
         plan_ng_guru)(1, &N1, 0, &n1, M, 2, NFFT_WINDOW_DIRAC_DELTA, x, fh, f,
                      0u, NFFT_ESTIMATE | NFFT_NO_FAST_NATIVE);
    CU_ASSERT_PTR_NOT_NULL(pu);
    Y(plan_ng_destroy)(pu);
  }
