...
typedef void (*destroy_t)(const testcase_delegate_t *ego_, int *N, R *x, C *f_hat, C *f);
...
static void destroy_file(const testcase_delegate_t *ego_, int *N, R *x, C *f_hat, C *f);
...
static init_delegate_t init_advanced_poly_psi;
static init_delegate_t init_advanced_poly_otf;
#if defined(GAUSSIAN)
...
static init_delegate_t init_advanced_poly_psi = {"init_guru (PRE PSI, poly)", init_advanced_pre_psi_, WINDOW_HELP_ESTIMATE_m, PRE_PHI_HUT | PRE_PSI | PRE_POLY_PSI | DEFAULT_NFFT_FLAGS, DEFAULT_FFTW_FLAGS};
static init_delegate_t init_advanced_poly_otf = {"init_guru (poly, no PRE PSI)", init_advanced_pre_psi_, WINDOW_HELP_ESTIMATE_m, PRE_PHI_HUT | PRE_POLY_PSI | DEFAULT_NFFT_FLAGS, DEFAULT_FFTW_FLAGS};
...
static const init_delegate_t* initializers_1d[] =
{
  ...
  &init_advanced_poly_psi,
  &init_advanced_poly_otf,
...
void X(check_acc)(void)
{
  check_many_file(SIZE(testcases_acc), SIZE(initializers_acc), SIZE(trafos_acc),
    testcases_acc_, initializers_acc, &check_trafo, trafos_acc);
}
