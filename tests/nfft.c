  ...
  check_delegate_t *check_delegate, const trafo_delegate_t **trafos)
{
  ...
  for (k = 0; k < nt; k++)
  {
    for (i = 0; i < nf; i++)
    {
      for (j = 0; j < ni; j++)
      {
         r = check_single_file(testcases[i], (const init_delegate_t *)initializers[j], check_delegate, (const trafo_delegate_t *)trafos[k], ABSPATH("nfft_accuracy.txt"));
         ok = MIN(ok, r);
      }
    }
  }
...
static const testcase_delegate_t **testcases_acc_ = (const testcase_delegate_t**)testcases_acc;
...
void X(check_acc)(void)
{
  check_many_file(SIZE(testcases_acc), SIZE(initializers_acc), SIZE(trafos_acc),
    (const testcase_delegate_t **)testcases_acc, (const init_delegate_t **)initializers_acc, &check_trafo, (const trafo_delegate_t **)trafos_acc);
}
