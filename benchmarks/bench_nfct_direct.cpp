...
static void init_random_data(NFCT(plan)* plan) {
...
// Benchmark for NFCT direct transform (trafo_direct)
static void nfct_forward_direct_1d(benchmark::State& state) {
    int N = state.range(0);
    int M = state.range(1);

    NFCT(plan) plan;
    NFCT(init_1d)(&plan, N, M);
    init_random_data(&plan);

    for (auto _ : state) {
        NFCT(trafo_direct)(&plan);
    }
...
static void nfct_adjoint_direct_1d(benchmark::State& state) {
    int N = state.range(0);
    int M = state.range(1);
...
static void nfct_forward_direct_2d(benchmark::State& state) {
    int N1 = state.range(0);
    int N2 = state.range(1);
    int M = state.range(2);

    NFCT(plan) plan;
    NFCT(init_2d)(&plan, N1, N2, M);
    init_random_data(&plan);
...
static void nfct_adjoint_direct_2d(benchmark::State& state) {
    int N1 = state.range(0);
    int N2 = state.range(1);
    int M = state.range(2);
...
static void nfct_forward_direct_3d(benchmark::State& state) {
    int N1 = state.range(0);
    int N2 = state.range(1);
...
static void nfct_adjoint_direct_3d(benchmark::State& state) {
    int N1 = state.range(0);
    int N2 = state.range(1);
    int N3 = state.range(2);
    int M = state.range(3);
