...
**Full methodology (commands, caveats, tooling status):** [REFERENCE.md](REFERENCE.md).
Read it before starting. This SKILL is the enforced checklist; REFERENCE.md is the detail.
...
- [ ] **Preflight — pick the measurement track.** CodSpeed fully available (MCP present
...
- [ ] **Phase 0 — baseline.** Build the self-contained CMake tree in the chosen mode and
...
- [ ] **Phase A — pin the correctness net [HARD GATE].** Inject the smallest fault into
  ...
  region is uncovered ⇒ stop** (try a more destructive fault first; then add a covering
  test or report the gap — never optimize uncovered code). Revert, confirm green, `git diff` empty.

- [ ] **Phase B — pin the performance metric [HARD GATE].** Measure the target's
  ...
  benchmark moves (or tooling can't measure) ⇒ stop** — add a benchmark or pick a real
  target. (Note: `init`-only code and the *fast* NFFT path have no benchmark; only
  `*_direct` transforms are covered.) Revert, `git diff` empty.

- [ ] **Phase C — inner loop.** Optimize against the *narrow* A-net + B-metric only.
...
- [ ] **Phase D — exit gate.** Re-run the ENTIRE Phase-0 baseline (full `ctest` + ALL
...
- **Compare medians, never single runs** (walltime is noisy in the container). Untouched,
