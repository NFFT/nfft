...
/*! \file nfft3util.h
 *  \brief Shared helper header for example and application programs.
 *
 *  This header sits *between* the public library header nfft3.h and the
 *  internal header infft.h. It exposes the precision-agnostic machinery that
 *  programs which are not part of the library object itself (examples,
 *  applications, benchmarks) need, while deliberately omitting the deeper
 *  internal machinery (window-function macros, timing macros coupled to plan
 *  structs, stack allocation, internal linear-algebra kernels, assertions)
 *  that lives in infft.h.
 *
 *  What you get here:
 *    - the precision-agnostic real/complex types \c R, \c C and the integer
 *      type \c INT;
 *    - the name-mangling macros \c Y / \c X / \c NFFT / \c NFCT / \c NFST /
 *      \c NFSFT / \c SOLVER / \c FFTW;
 *    - precision-aware literals and constants (\c K, \c KPI, \c K2PI, ...);
 *    - the precision-agnostic math-function macros (\c POW, \c SIN, \c SQRT,
 *      \c CEXP, \c CABS, ...);
 *    - format-string helpers (\c __FE__, \c __FES__, ...);
 *    - timing (\c Y(elapsed_seconds)) and a selection of math/error/quadrature
 *      utility prototypes from the \c util module.
 *
 *  Like infft.h, this header is precision-driven by config.h (NFFT_SINGLE /
 *  NFFT_LDOUBLE) and is therefore *not* installed; it is for in-tree builds.
 *  It is distinct from the installed, consumer-facing nfft3mp.h, which provides
 *  a precision-agnostic layer using \c NFFT_-prefixed names for external code.
 *
 *  infft.h includes this header and adds the remaining internal machinery, so
 *  there is a single source of truth for the definitions below.
 */
...
/* Public library API (types, plan structs, transform prototypes). */
#include "nfft3.h"
...
/* Precision-agnostic math functions (extern declarations + name mapping). */
#if defined(NFFT_LDOUBLE)
...
/* Return number of elapsed seconds between two time points (util/time.c). */
/** Return number of elapsed seconds between two time points. */
R Y(elapsed_seconds)(ticks t1, ticks t0);

/* Selected util-module helpers usable by example/application programs. */
/* sinc.c: */
